...
export class SQLiteAdapter implements PersistenceAdapter {
  ...
  getTaskOutput(taskId: string): string {
    ...
    // still appended because explicit diagnostics (startup/shutdown/fix notes)
    // are written there even when streamed output exists. Legacy task_output
    // rows are only used as a fallback to avoid replaying duplicated stream
