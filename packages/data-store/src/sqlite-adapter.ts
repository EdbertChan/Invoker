...
export class SQLiteAdapter implements PersistenceAdapter {
  ...
  getTaskOutput(taskId: string): string {
    ...
    // it is the canonical streaming-output store. Current diagnostic writes
    // are file-backed task output, so append that file after the stream to keep
    // shutdown/startup context visible in persisted retrieval. Otherwise fall
    // back to task_output (legacy DB rows + diagnostic file), which avoids
    // returning a duplicated legacy stream when both SQLite tables contain the
    // same data.
