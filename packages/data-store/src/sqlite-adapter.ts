...
import { loadNativeSqlite, NativeDatabaseCompat, sqlStringLiteral } from './sqlite-connection.js';
import { SQLiteQueryHelper } from './sqlite-query.js';
import { initializeSQLiteSchema, runSQLiteSchemaMigrations } from './sqlite-schema.js';
import { SQLiteWriteHelper } from './sqlite-write.js';
...
export class SQLiteAdapter implements PersistenceAdapter {
  ...
  private queries: SQLiteQueryHelper;
  private writes: SQLiteWriteHelper;
  ...
  private constructor(db: DatabaseSync, dbPath: string | null, options?: SQLiteAdapterOptions) {
    ...
    this.queries = new SQLiteQueryHelper(this.db);
    this.writes = new SQLiteWriteHelper(
      this.db,
      () => this.readOnly,
      () => {
        this.dirty = true;
  ...
  private execRun(sql: string, params: unknown[] = []): void {
    this.writes.execRun(sql, params);
  }

  private runTransaction<T>(work: () => T): T {
    return this.writes.runTransaction(work);
  }
  ...
  private migrate(): void {
    runSQLiteSchemaMigrations({
      ...
      migrateTestCommands: () => this.migrateTestCommands(),
      migrateGatePolicyApprovedToCompleted: () => this.migrateGatePolicyApprovedToCompleted(),
      migrateTaskExternalDependenciesToWorkflows: () => this.migrateTaskExternalDependenciesToWorkflows(),
      runCompatibilityMigration: () => {
