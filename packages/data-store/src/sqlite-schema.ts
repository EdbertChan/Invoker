...
export interface SQLiteCompatibilityMigrationReport {
...
export function configureSQLiteConnection(db: DatabaseSync, fileBacked: boolean): void {
...
export function initSQLiteSchema(db: NativeDatabaseCompat): void {
  db.run(`
...
export function migrateSQLiteSchema(ctx: SQLiteSchemaContext): void {
  const migrations = [
  ...
  migrateWorkflowStatusColumn(ctx);

  ctx.db.run('DROP INDEX IF EXISTS idx_attempts_node');
  ctx.db.run('CREATE INDEX IF NOT EXISTS idx_attempts_node_created ON attempts(node_id, created_at)');
  ctx.db.run('CREATE INDEX IF NOT EXISTS idx_tasks_workflow_id ON tasks(workflow_id)');

  if (!ctx.isReadOnly()) {
    migrateTestCommands(ctx);
    migrateGatePolicyApprovedToCompleted(ctx);
    runSQLiteCompatibilityMigration(ctx);
  }
}

export function runSQLiteCompatibilityMigration(ctx: SQLiteSchemaContext): SQLiteCompatibilityMigrationReport {
  const report = {
  ...
  ctx.runTransaction(() => {
    ...
    report.migratedFixingWithAiStatuses = ctx.db.getRowsModified();
    ...
    report.normalizedMergeModes = ctx.db.getRowsModified();

    const staleAutoFixRows = ctx.queryAll(
    ...
    report.normalizedStaleLaunchMetadata = ctx.db.getRowsModified();

    const missingSshPoolRows = ctx.queryAll(
    ...
    for (const row of missingSshPoolRows) {
      ...
      report.backfilledMissingSshPoolMemberIds += ctx.db.getRowsModified();
    }
  });
  return report;
}

function migrateWorkflowStatusColumn(ctx: SQLiteSchemaContext): void {
  if (ctx.isReadOnly()) return;
  const columns = ctx.queryAll('PRAGMA table_info(workflows)') as Array<{ name: string }>;
  if (!columns.some((column) => column.name === 'status')) return;

  const foreignKeys = ctx.queryOne('PRAGMA foreign_keys') as { foreign_keys?: number } | undefined;
  const foreignKeysEnabled = foreignKeys?.foreign_keys === 1;
  if (foreignKeysEnabled) {
    ctx.db.run('PRAGMA foreign_keys = OFF');
  }
  ctx.db.run(`
  ...
  ctx.db.run(`
    ...
    FROM workflows
  `);
  ctx.db.run('DROP TABLE workflows');
  ctx.db.run('ALTER TABLE workflows_new RENAME TO workflows');
  if (foreignKeysEnabled) {
...
function migrateTestCommands(ctx: SQLiteSchemaContext): void {
  try {
    const rows = ctx.queryAll(
    ...
    for (const row of rows) {
      ...
      if (fixed !== row.command) {
        ctx.execRun('UPDATE tasks SET command = ? WHERE id = ?', [fixed, row.id]);
      }
    }
  } catch {
...
function migrateGatePolicyApprovedToCompleted(ctx: SQLiteSchemaContext): void {
  try {
    const rows = ctx.queryAll(
    ...
    for (const row of rows) {
      try {
        ...
        if (modified) {
          ...
          ctx.execRun('UPDATE tasks SET external_dependencies = ? WHERE id = ?', [updated, row.id]);
        }
      } catch {
