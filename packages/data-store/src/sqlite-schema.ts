...
type QueryOne = (sql: string, params?: unknown[]) => Record<string, unknown> | undefined;
type QueryAll = (sql: string, params?: unknown[]) => Record<string, unknown>[];

interface SQLiteSchemaMigrationHooks {
  db: NativeDatabaseCompat;
  readOnly: boolean;
  queryOne: QueryOne;
  queryAll: QueryAll;
  markDirty: () => void;
  migrateTestCommands: () => void;
  migrateGatePolicyApprovedToCompleted: () => void;
  migrateTaskExternalDependenciesToWorkflows: () => void;
  runCompatibilityMigration: () => void;
}
...
export function runSQLiteSchemaMigrations(hooks: SQLiteSchemaMigrationHooks): void {
  const migrations = [
  ...
  migrateWorkflowStatusColumn(hooks);

  // Replace old attempt_number index with created_at index
  hooks.db.run('DROP INDEX IF EXISTS idx_attempts_node');
  hooks.db.run('CREATE INDEX IF NOT EXISTS idx_attempts_node_created ON attempts(node_id, created_at)');
  hooks.db.run('CREATE INDEX IF NOT EXISTS idx_tasks_workflow_id ON tasks(workflow_id)');
  hooks.db.run('DROP INDEX IF EXISTS idx_task_launch_dispatch_active_attempt');
  hooks.db.run(`
    CREATE UNIQUE INDEX IF NOT EXISTS idx_task_launch_dispatch_active_attempt
      ON task_launch_dispatch(attempt_id)
      WHERE state IN ('enqueued', 'leased')
  ...
  if (!hooks.readOnly) {
    hooks.migrateTestCommands();
    hooks.migrateGatePolicyApprovedToCompleted();
    hooks.migrateTaskExternalDependenciesToWorkflows();
    hooks.runCompatibilityMigration();
  }
}

function migrateWorkflowStatusColumn(hooks: SQLiteSchemaMigrationHooks): void {
  if (hooks.readOnly) return;
  const columns = hooks.queryAll('PRAGMA table_info(workflows)') as Array<{ name: string }>;
  if (!columns.some((column) => column.name === 'status')) return;

  const foreignKeys = hooks.queryOne('PRAGMA foreign_keys') as { foreign_keys?: number } | undefined;
  const foreignKeysEnabled = foreignKeys?.foreign_keys === 1;
  if (foreignKeysEnabled) {
    hooks.db.run('PRAGMA foreign_keys = OFF');
  }
  hooks.db.run(`
  ...
  hooks.db.run(`
    ...
    FROM workflows
  `);
  hooks.db.run('DROP TABLE workflows');
  hooks.db.run('ALTER TABLE workflows_new RENAME TO workflows');
  if (foreignKeysEnabled) {
    hooks.db.run('PRAGMA foreign_keys = ON');
  }
  hooks.markDirty();
}
