...
export interface SQLiteSchemaMigrationContext {
  db: SqlJsDatabase;
  readOnly: boolean;
...
export function migrateSchema(ctx: SQLiteSchemaMigrationContext): void {
  const migrations = [
  ...
  if (!ctx.readOnly) {
    ...
    ctx.runCompatibilityMigration();
  }
}

function migrateTestCommands(ctx: SQLiteSchemaMigrationContext): void {
  try {
...
function migrateGatePolicyApprovedToCompleted(ctx: SQLiteSchemaMigrationContext): void {
  try {
    const rows = ctx.queryAll(
