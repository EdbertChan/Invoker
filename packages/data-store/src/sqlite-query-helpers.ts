...
export interface SQLiteQueryStatement {
  get(...params: unknown[]): Record<string, unknown> | undefined;
  all(...params: unknown[]): Record<string, unknown>[];
  free(): void;
}

export interface SQLiteQueryDatabase {
  prepare(sql: string): SQLiteQueryStatement;
}

export function paramsToArgs(params: unknown[] = []): unknown[] {
  return Array.isArray(params) ? params : [params];
}

/** Run a single-row SELECT, returning the row as an object or undefined. */
export function queryOne(
  db: SQLiteQueryDatabase,
  sql: string,
  params: unknown[] = [],
): Record<string, unknown> | undefined {
  const stmt = db.prepare(sql);
  try {
    return stmt.get(...paramsToArgs(params)) as Record<string, unknown> | undefined;
  } finally {
    stmt.free();
  }
}
...
export function queryAll(
  ...
  const stmt = db.prepare(sql);
  try {
    return stmt.all(...paramsToArgs(params)) as Record<string, unknown>[];
  } finally {
    stmt.free();
  }
}

export function loadWorkflowRollups(
  queryAllRows: (sql: string, params?: unknown[]) => Record<string, unknown>[],
  workflowIds: string[],
  ...
  return computeWorkflowRollupsFromRows(workflowIds, taskRows);
}

export function computeWorkflowRollupsFromRows(
  workflowIds: string[],
...
export function rowToWorkflow(row: any, rollup?: WorkflowRollup): Workflow {
  return {
    id: row.id,
    name: row.name,
...
export function rowToTask(row: any): TaskState {
...
export function reconcileTaskFromSelectedAttempt(
  task: TaskState,
...
export function rowToAttempt(row: any): Attempt {
  return {
    id: row.id,
    nodeId: row.node_id,
