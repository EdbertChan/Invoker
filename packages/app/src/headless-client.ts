...
const LIVE_OWNER_QUERY_SUBCOMMANDS = new Set(['ui-perf', 'queue']);
...
async function delegateReadOnlyQuery(
  ...
  const liveQuery = args[0] === 'query' && LIVE_OWNER_QUERY_SUBCOMMANDS.has(args[1] ?? '')
    ? args[1]
    : args[0] === 'queue'
      ? 'queue'
      : undefined;
  ...
  const isUiPerf = liveQuery === 'ui-perf';
