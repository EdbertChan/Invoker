...
export function createTaskRunner(deps: TaskRunnerWiringDeps): TaskRunner {
  const taskExecutor = new TaskRunner({
  ...
  wireApproveHook(deps);
  return taskExecutor;
}

function wireApproveHook(deps: TaskRunnerWiringDeps): void {
  deps.orchestrator.setBeforeApproveHook(async (task) => {
    if (task.config.isMergeNode && task.config.workflowId && task.execution.pendingFixError === undefined) {
      ...
      await deps.getTaskExecutor().approveMerge(task.config.workflowId);
    }
  });
}
