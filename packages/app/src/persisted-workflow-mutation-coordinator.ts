...
export class PersistedWorkflowMutationCoordinator {
  ...
  constructor(
    ...
    private readonly dispatch: (channel: string, args: unknown[], signal: AbortSignal) => Promise<unknown>,
  ...
  private async executeIntent(workflowId: string, intent: WorkflowMutationIntent): Promise<void> {
    ...
    try {
      ...
      const dispatchPromise = Promise.resolve(this.dispatch(intent.channel, intent.args, invalidation.abortController.signal));
      ...
      if (!invalidation.abortController.signal.aborted) {
