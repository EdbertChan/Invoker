...
describe('PersistedWorkflowMutationCoordinator', () => {
  ...
  it('evicts older queued workflow intents when invoker:recreate-with-rebase fence starts', async () => {
    const adapter = await SQLiteAdapter.create(':memory:');
    adapters.push(adapter);
    adapter.saveWorkflow({
      id: 'wf-1',
  ...
  it('invalidates an older running workflow intent when invoker:recreate-with-rebase is enqueued', async () => {
    const adapter = await SQLiteAdapter.create(':memory:');
    adapters.push(adapter);
    adapter.saveWorkflow({
      id: 'wf-1',
    ...
    const intentsAfter = adapter.listWorkflowMutationIntents('wf-1');
    expect(intentsAfter.find((intent) => intent.id === 1)?.status).toBe('failed');
    expect(intentsAfter.find((intent) => intent.id === 1)?.error).toContain('Superseded by recreate intent #2');
    expect(intentsAfter.find((intent) => intent.id === 2)?.status).toBe('completed');
    expect(order).toEqual([
      'invoker:fix-with-agent:wf-1/blocker-task',
      'invoker:recreate-with-rebase:wf-1',
    ]);
