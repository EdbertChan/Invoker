...
describe('PersistedWorkflowMutationCoordinator', () => {
  ...
  it('invalidates an older running workflow intent when IPC recreate-task is enqueued', async () => {
    const adapter = await SQLiteAdapter.create(':memory:');
    adapters.push(adapter);
    adapter.saveWorkflow({
      id: 'wf-1',
  ...
  it('invalidates an older running workflow intent when delegated recreate-task is enqueued', async () => {
    const adapter = await SQLiteAdapter.create(':memory:');
    adapters.push(adapter);
    adapter.saveWorkflow({
      id: 'wf-1',
  ...
  it('evicts older queued workflow intents when IPC recreate-task fence starts', async () => {
    const adapter = await SQLiteAdapter.create(':memory:');
    adapters.push(adapter);
    adapter.saveWorkflow({
      id: 'wf-1',
