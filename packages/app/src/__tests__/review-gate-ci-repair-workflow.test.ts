...
import {
  spawnReviewGateCiRepairWorkflow,
  type ReviewGateCiRepairWorkflowMutationArgs,
...
const POLL_CADENCE_RULE = "- Check the backgrounded verify/CI command's status no more than once every ~180 seconds; if it has not finished after about 15 such checks, stop polling and report the situation instead of continuing to poll indefinitely.";
...
describe('spawnReviewGateCiRepairWorkflow', () => {
  ...
  it('spawns one repair workflow and leaves the source merge gate untouched', async () => {
    ...
    expect(repairTasks[0]!.config.prompt).toEqual(expect.stringContaining(POLL_CADENCE_RULE));
