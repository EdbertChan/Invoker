...
describe('buildInvalidationDeps', () => {
  ...
  // Cross-workflow cascade wiring (Phase 3.3 of "Cascade Upstream
  // Invalidations to Downstream Workflows"). The dep MUST resolve a
  // task-scoped id to its owning workflowId via
  // `orchestrator.getTask(id)?.config.workflowId`, then delegate to
