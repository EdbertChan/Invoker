...
describe('finalizeAppliedFix', () => {
  it('leaves task awaiting approval when autoApproveAIFixes is disabled', async () => {
    ...
    expect(orchestrator.setFixAwaitingApproval).toHaveBeenCalledWith('task-a', 'saved-error', undefined);
    expect(orchestrator.approve).not.toHaveBeenCalled();
  });
...
describe('autoFixOnFailure', () => {
  ...
  it('uses fixWithAgent for non-merge failures and restarts the task directly', async () => {
    ...
    expect(orchestrator.beginConflictResolution).toHaveBeenCalledWith(
      'task-a',
      { taskId: 'task-a', selectedAttemptId: undefined, generation: 0 },
    );
...
describe('fixWithAgentAction lineage guard', () => {
  it('discards the result when lineage changes during async fix', async () => {
    let fixCallCount = 0;
    const orchestrator = {
      getTask: vi.fn(() => {
        fixCallCount++;
  ...
  it('discards the result when signal is aborted during async fix', async () => {
    const ac = new AbortController();
    const orchestrator = {
      getTask: vi.fn(() => makeTask({
        status: 'failed',
...
describe('autoFixOnReviewGateFailure', () => {
  ...
  it('starts a review-gate fix session and passes CI context to the fix agent', async () => {
    ...
    expect(orchestrator.setFixAwaitingApproval).toHaveBeenCalledWith(
      'merge-a',
      'Review-gate CI failed',
      { taskId: 'merge-a', selectedAttemptId: 'att-1', generation: 7 },
    );
  });
