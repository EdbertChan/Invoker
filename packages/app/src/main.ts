...
// ══════════════════════════════════════════════════════════════
  ...
  app.on('second-instance', mainWindowLifecycle.focusMainWindowForSecondInstance);
  ...
  function rebuildTaskRunner(): void {
    ...
    wireTaskRunnerApproveHook({
      orchestrator,
      persistence,
      requireTaskExecutor: () => requireTaskExecutor(),
    });
  }
  ...
  app.whenReady().then(async () => {
    ...
    await bootstrapApp({
      ...
      createWindow,
      recordStartupMark,
      bindActivationHandler: mainWindowLifecycle.bindActivationHandler,
    });
  }).catch((err) => {
    process.stderr.write(`${RED}Error:${RESET} ${err instanceof Error ? err.message : String(err)}\n`);
    app.quit();
