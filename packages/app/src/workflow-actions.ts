...
export async function approveTask(
  ...
  guard?: { lineage?: TaskLineageSnapshot; signal?: AbortSignal },
): Promise<ApproveTaskResult> {
  if (guard?.lineage) assertLineageCurrent(guard.lineage, deps.orchestrator, guard.signal);
  ...
  const started = shouldResume
    ? await (
      deps.resumeAfterFixApproval
        ? deps.resumeAfterFixApproval(taskId)
        : guard?.lineage
          ? deps.orchestrator.resumeTaskAfterFixApproval(taskId, guard.lineage)
          : deps.orchestrator.resumeTaskAfterFixApproval(taskId)
    )
    : await (
      deps.approve
        ? deps.approve(taskId)
        : guard?.lineage
          ? deps.orchestrator.approve(taskId, guard.lineage)
          : deps.orchestrator.approve(taskId)
    );
