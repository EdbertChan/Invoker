...
export async function fixWithAgentAction(
  ...
  try {
    if (recoveryRoute.kind === 'resolveConflict') {
      ...
      await taskExecutor.fixWithAgent(taskId, output, options.agentName, persistedSavedError, options.signal);
    }
...
export async function autoFixOnFailure(
  ...
  try {
    ...
    if (recoveryRoute.kind === 'resolveConflict') {
      ...
      const output = persistence.getTaskOutput(taskId);
      await taskExecutor.fixWithAgent(taskId, output, agentSelection.selectedAgent, persistedSavedError, deps.signal);
    }
