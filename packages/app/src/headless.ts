  ...
  for (const wf of targetWorkflows) {
    ...
    for (const task of tasks) {
      const ctx = buildAttributionContext({
        id: task.id,
        workflowId: wf.id,
        executorType: task.config.dockerImage ? 'docker' : (task.config.poolId ? 'pool' : 'worktree'),
...
export async function runHeadless(args: string[], deps: HeadlessDeps): Promise<void> {
  ...
  switch (command) {
    ...
    case 'edit-executor':
    case 'edit-type':
      throw new Error(`${command} is no longer supported. Use "set pool <taskId> <poolId>" instead.`);
      break;
    case 'edit-agent':
...
${BOLD}Configure:${RESET}
  ...
  set pool <taskId> <poolId>                          Change execution pool
...
async function headlessEditPool(
  ...
  process.stdout.write(
    `Edited task "${taskId}" pool → "${poolId}"\n`,
  );
  ...
  if (driver.fetchRemoteSession && allTasks) {
    const sshTask = allTasks.find(
      t => t.execution.agentSessionId === sessionId
        && Boolean(t.config.poolId),
    );
    if (sshTask) {
      const { loadConfig } = await import('./config.js');
      const config = loadConfig();
      const targets = config.remoteTargets ?? {};
      const pool = sshTask.config.poolId
        ? config.executionPools?.[sshTask.config.poolId]
        : undefined;
      const sshMember = pool?.members.find((member) => member.type === 'ssh');
      const target = sshMember ? targets[sshMember.id] : undefined;
      if (target) {
