...
import { remoteFetchForPool } from '@invoker/execution-engine';
...
import { handleHeadlessQuery, handleHeadlessQuerySelect } from './headless-query-list.js';
import { handleHeadlessRun, handleHeadlessResume } from './headless-run-resume.js';
import {
  handleHeadlessApprove,
  handleHeadlessCancel,
  handleHeadlessCancelWorkflow,
  handleHeadlessDeleteAll,
  handleHeadlessDeleteWorkflow,
  handleHeadlessInput,
  handleHeadlessReject,
  handleHeadlessSelect,
} from './headless-approve-delete.js';

export type { HeadlessAutoFixController, HeadlessDeps } from './headless-shared.js';
export { createHeadlessExecutor, wireHeadlessApproveHook, wireHeadlessAutoFix } from './headless-shared.js';
...
export async function runHeadless(args: string[], deps: HeadlessDeps): Promise<void> {
  ...
  switch (command) {
    ...
    case 'query':
      await handleHeadlessQuery(args.slice(1), deps);
      break;
    case 'set':
    ...
    // ── Execute (unchanged) ──
    case 'run':
      await handleHeadlessRun(args[1], deps, deps.waitForApproval, deps.noTrack);
      break;
    case 'resume':
      await handleHeadlessResume(args[1], deps, deps.waitForApproval, deps.noTrack);
      break;
    case 'retry':
    ...
    // ── Respond (unchanged) ──
    case 'approve':
      await handleHeadlessApprove(args[1], deps);
      break;
    case 'reject':
      await handleHeadlessReject(args[1], deps, args.slice(2).join(' ') || undefined);
      break;
    case 'input':
      await handleHeadlessInput(args[1], args.slice(2).join(' '), deps);
      break;
    case 'select':
      await handleHeadlessSelect(args[1], args[2], deps);
      break;

    // ── Lifecycle (unchanged) ──
    case 'cancel':
      await handleHeadlessCancel(args[1], deps);
      break;
    case 'cancel-workflow':
      await handleHeadlessCancelWorkflow(args[1], deps);
      break;
    case 'delete':
    case 'delete-workflow':
      await handleHeadlessDeleteWorkflow(args[1], deps);
      break;
    case 'delete-all':
      await handleHeadlessDeleteAll(deps);
      break;
    case 'open-terminal':
    ...
    case 'slack':
      ...
      break;
    case 'query-select':
      await handleHeadlessQuerySelect(args[1], deps);
      break;

    // ── Deprecated aliases → query ──
    case 'list':
      warnDeprecated('list', 'query workflows');
      await handleHeadlessQuery(['workflows', ...args.slice(1)], deps);
      break;
    case 'status':
      warnDeprecated('status', 'query tasks');
      await handleHeadlessQuery(['tasks', ...args.slice(1)], deps);
      break;
    case 'task-status':
      warnDeprecated('task-status', 'query task');
      await handleHeadlessQuery(['task', ...args.slice(1)], deps);
      break;
    case 'queue':
      warnDeprecated('queue', 'query queue');
      await handleHeadlessQuery(['queue', ...args.slice(1)], deps);
      break;
    case 'audit':
      warnDeprecated('audit', 'query audit');
      await handleHeadlessQuery(['audit', ...args.slice(1)], deps);
      break;
    case 'session':
      warnDeprecated('session', 'query session');
      await handleHeadlessQuery(['session', ...args.slice(1)], deps);
      break;
