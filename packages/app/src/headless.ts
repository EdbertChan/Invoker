...
import { handleHeadlessQueryCommand } from './headless-query-commands.js';
import { handleHeadlessRunCommand } from './headless-run-commands.js';
import { handleHeadlessApproveDeleteCommand } from './headless-approve-delete-commands.js';
import {
  BOLD,
  RESET,
...
export {
  parseQueryFlags,
  resolveAgentSession,
} from './headless-query-commands.js';
...
export async function runHeadless(args: string[], deps: HeadlessDeps): Promise<void> {
  ...
  if (await handleHeadlessRunCommand(args, deps)) return;
  if (await handleHeadlessQueryCommand(args, deps)) return;
  if (await handleHeadlessApproveDeleteCommand(args, deps)) return;
