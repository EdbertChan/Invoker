...
async function headlessFix(taskId: string, deps: HeadlessDeps, agentArg?: string): Promise<void> {
  ...
  try {
    ...
    if (err instanceof StaleLineageError) {
      deps.logger.info(`headless fix discarded stale result for task "${taskId}": ${err.message}`, { module: 'headless' });
      return;
    }
    await finalizeMutationWithGlobalTopup({
      orchestrator: deps.orchestrator,
...
async function headlessResolveConflict(taskId: string, deps: HeadlessDeps, agentArg?: string): Promise<void> {
  ...
  try {
    ...
    if (err instanceof StaleLineageError) {
      deps.logger.info(`headless resolve-conflict discarded stale result for task "${taskId}": ${err.message}`, { module: 'headless' });
      return;
    }
    await finalizeMutationWithGlobalTopup({
      orchestrator: deps.orchestrator,
