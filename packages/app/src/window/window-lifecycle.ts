...
export function createMainWindow(deps: MainWindowLifecycleDeps): BrowserWindow {
  ...
  const mainWindow = new deps.browserWindow({
    ...
    icon: !icon.isEmpty() && deps.platform !== 'darwin' ? icon : undefined,
    title: 'Invoker',
  });
  ...
  const devUrl = deps.env.VITE_DEV_SERVER_URL;
  if (devUrl) {
  ...
  const shouldShowWindow = deps.env.NODE_ENV !== 'test' || deps.enableTestCompositor;
  if (shouldShowWindow) {
  ...
  mainWindow.webContents.setWindowOpenHandler(({ url }) => {
    if (url.startsWith('https://') || url.startsWith('http://')) {
      ...
      if (browserCmd) {
        ...
        const chromeCmd: [string, string[]] = deps.platform === 'darwin'
