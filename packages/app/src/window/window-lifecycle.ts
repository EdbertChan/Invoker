...
export function registerSecondInstanceFocus(deps: Pick<MainWindowLifecycleDeps, 'app' | 'getMainWindow'>): void {
...
export function createMainWindow(deps: MainWindowLifecycleDeps): BrowserWindow {
  ...
  mainWindow.webContents.setWindowOpenHandler(({ url }) => {
    if (url.startsWith('https://') || url.startsWith('http://')) {
      const browserCmd = deps.browserCommand;
      if (browserCmd) {
...
export function registerActivationHandler(deps: ActivationHandlerDeps): void {
...
export function registerWindowAllClosedHandler(deps: Pick<MainWindowLifecycleDeps, 'app' | 'logger'>): void {
