...
function WorkflowGraphInner({
  ...
  const edges = useMemo<Edge[]>(() => graph.edges.map((edge) => ({
    ...
    animated: false,
    style: {
      stroke: edge.kind === 'detached' ? 'rgba(245,158,11,0.5)' : 'rgba(148,163,184,0.55)',
      strokeWidth: edge.kind === 'detached' ? 1.5 : 2,
      strokeDasharray: edge.kind === 'detached' ? '6 6' : undefined,
    },
    data: { kind: edge.kind },
    ariaLabel: edge.kind === 'detached' ? 'Detached workflow lineage' : 'Active workflow dependency',
    zIndex: 0,
    markerEnd: {
      type: MarkerType.ArrowClosed,
      color: edge.kind === 'detached' ? 'rgba(245,158,11,0.5)' : 'rgba(148,163,184,0.55)',
      width: 16,
      height: 16,
    },
  })), [graph.edges]);
