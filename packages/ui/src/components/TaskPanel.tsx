...
export function TaskPanel({
  ...
  const canEditPrompt = task.config.prompt !== undefined && task.status !== 'running' && task.status !== 'fixing_with_ai' && onEditPrompt;
          ...
          {isEditingCommand && task.config.command !== undefined ? (
            ...
            <div className="mt-2 space-y-2">
              <textarea
                ...
                className="w-full rounded p-3 text-xs text-blue-300 bg-blue-900/20 border border-blue-500 focus:outline-none focus:border-blue-400 resize-y"
                rows={4}
            ...
            <div
              className={`mt-2 rounded p-3 text-xs select-text ${
                task.config.prompt
                  ? canEditPrompt
                    ? 'bg-blue-900/20 border border-blue-800 cursor-pointer hover:border-blue-700 transition-colors'
