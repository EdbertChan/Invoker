...
export function MergeGateNode({ data }: MergeGateNodeProps) {
  ...
  /** approvalMode wins over gateKind so we never show "Pull request" + "Review" when workflow is external_review mode. */
  const effectiveGateKind: MergeGateKind = approvalMode === 'external_review' ? 'external_review' : gateKind;
  ...
  return (
    ...
    >
      ...
      {approvalMode === 'manual' && (status === 'review_ready' || status === 'awaiting_approval') && (
        <button
