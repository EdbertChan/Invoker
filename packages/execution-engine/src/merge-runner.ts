...
function renderTemplate(templatePath: string, substitutions: Record<string, string>): string {
  ...
  // Collapse runs of 3+ consecutive newlines to exactly 2 newlines
...
function buildTestPlanSection(workflowTasks: TaskState[]): string {
  const tasksWithCommand = workflowTasks.filter((t) => t.config.command);
  if (tasksWithCommand.length === 0) return '';
  const lines: string[] = [];
  lines.push('## Test Plan');
  lines.push('');
  for (const t of tasksWithCommand) {
...
function buildArchitectureSection(
  ...
  if (workflowTasks.length >= 3) {
    ...
    for (const t of workflowTasks) {
      const sid = sanitizeMermaidId(t.id);
      if (t.status === 'completed') {
        lines.push(`  style ${sid} fill:#c8e6c9`);
      } else if (t.status === 'failed') {
        lines.push(`  style ${sid} fill:#ffcdd2`);
      }
    }
    lines.push('```');
    lines.push('');
...
export async function buildMergeSummaryImpl(
  ...
  // SUMMARY_DESCRIPTION includes trailing separator when present, empty when absent
  const summaryDescription = description && description.trim()
    ? `${description}\n\n---\n`
    : '';
