...
export async function buildMergeSummaryImpl(
  ...
  const templatePath = fileURLToPath(
    new URL(join('..', '..', '..', 'templates', 'pr-body.md'), import.meta.url),
  );
...
async function buildFileChangesSection(
  completed: TaskState[],
  workflow: ReturnType<SQLiteAdapter['loadWorkflow']>,
  host: MergeRunnerHost,
): Promise<string> {
  ...
  for (const t of completed) {
    if (!t.execution.branch) continue;
...
function buildTestPlanSection(workflowTasks: TaskState[]): string {
  ...
  for (const t of commandTasks) {
    ...
    if (t.status === 'completed') {
      lines.push(`- [x] \`${command}\` — passed`);
    } else if (t.status === 'failed') {
      lines.push(`- [ ] \`${command}\` — failed (exit ${t.execution.exitCode ?? 'unknown'})`);
    }
  }
  return lines.join('\n');
}
...
function buildArchitectureSection(
  ...
  if (workflowTasks.length < 3) return '';
  ...
  for (const t of workflowTasks) {
    ...
    const label = t.description.length > 50 ? `${t.description.slice(0, 47)}...` : t.description;
