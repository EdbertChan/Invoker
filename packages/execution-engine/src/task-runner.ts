...
import { prepareTaskLaunch } from './task-runner-prepare.js';
import { dispatchTaskExecution } from './task-runner-dispatch.js';
import { finalizeTaskLaunchFailure, settleTaskLaunch } from './task-runner-finalize.js';
...
export class TaskRunner {
  ...
  async executeTask(task: TaskState, dispatchOpts?: LaunchDispatchOptions): Promise<void> {
    const prepared = prepareTaskLaunch(this, task, dispatchOpts);
    const { attemptId, startGeneration, bench } = prepared;
    if (!prepared.accepted) return;
    try {
      await this.executeTaskInner(task, attemptId, bench, dispatchOpts);
      bench('executeTask.innerReturned');
    } catch (err) {
      await finalizeTaskLaunchFailure(this, task, attemptId, startGeneration, bench, err, dispatchOpts);
    } finally {
      settleTaskLaunch(this, attemptId, bench);
    }
  }
  ...
  private async executeTaskInner(
    ...
    return dispatchTaskExecution(this, task, attemptId, bench, dispatchOpts);
  }
