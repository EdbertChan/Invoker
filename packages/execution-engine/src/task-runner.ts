...
export class TaskRunner {
  ...
  /** @internal */ async ensureRepoMirrorPath(repoUrl: string): Promise<string | undefined> {
    ...
    try {
      ...
      this.logger.warn('[merge] ensureRepoMirrorPath failed', {
        repoUrl: trimmed,
  ...
  async executeTask(task: TaskState): Promise<void> {
    ...
    try {
      ...
      this.logger.error('[TaskRunner] executeTask failed', {
        taskId: task.id,
  ...
  private async executeTaskInner(task: TaskState, attemptId: string): Promise<void> {
    ...
    if (!launchAccepted) {
      ...
      try {
        ...
        this.logger.warn('[TaskRunner] failed to kill rejected launch', {
          taskId: task.id,
      ...
      executor.onComplete(handle, async (response: WorkResponse) => {
        const work = async () => {
          ...
          try {
            ...
            this.logger.error('[TaskRunner] onComplete handler failed', {
              taskId: task.id,
              error: err instanceof Error ? (err.stack ?? err.message) : String(err),
            });
  ...
  /** @internal */ async createMergeWorktree(ref: string, label: string, repoUrl?: string): Promise<string> {
    ...
    if (repoUrl) {
      ...
      if (mirrorPath) {
        ...
        this.logger.warn('[createMergeWorktree] Pool mirror unavailable, falling back to host repo', {
          repoUrl,
        });
      }
    }
  ...
  resumeMergeGatePolling(): void {
    ...
    for (const task of this.orchestrator.getAllTasks()) {
      if (
        ...
        this.logger.info('[merge-gate] Resuming PR polling', {
          taskId: task.id,
    ...
    for (const task of this.orchestrator.getAllTasks()) {
      if (
        ...
        try {
          ...
          this.logger.error('[merge-gate] PR status check error', {
            taskId: task.id,
            error: err instanceof Error ? (err.stack ?? err.message) : String(err),
          });
        }
  ...
  /** @internal */ startPrPolling(taskId: string, reviewId: string, workflowId: string): void {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (status.approved) {
          this.logger.info('[merge-gate] PR approved, completing merge gate', {
            taskId,
            reviewId,
          });
          ...
          this.logger.info('[merge-gate] PR rejected', {
            taskId,
            reviewId,
            statusText: status.statusText,
          });
        ...
        this.logger.error('[merge-gate] PR poll error', {
          taskId,
          error: err instanceof Error ? (err.stack ?? err.message) : String(err),
        });
  ...
  async checkPrApprovalNow(taskId: string): Promise<void> {
    ...
    try {
      ...
      if (status.approved) {
        this.logger.info('[merge-gate] PR approved (manual check), completing merge gate', {
          taskId,
          reviewId,
        });
        this.stopPrPolling(taskId);
        ...
        this.logger.info('[merge-gate] PR rejected (manual check)', {
          taskId,
          reviewId,
          statusText: status.statusText,
        });
      ...
      this.logger.error('[merge-gate] Manual PR check error', {
        taskId,
        error: err instanceof Error ? (err.stack ?? err.message) : String(err),
      });
    }
  ...
  private async cleanupPerTaskDockerExecutor(task: TaskState): Promise<void> {
    ...
    try {
      ...
      this.logger.warn('[TaskRunner] cleanupPerTaskDockerExecutor destroyAll failed', {
        dockerKey,
        error: err instanceof Error ? err.message : String(err),
      });
    }
    ...
    try {
      for (const branch of taskBranches) {
        try {
          ...
          this.logger.info('[rebase] Successfully rebased', { branch, baseBranch });
        } catch (err) {
          const msg = err instanceof Error ? err.message : String(err);
          errors.push(`${branch}: ${msg}`);
          this.logger.error('[rebase] Failed to rebase', { branch, baseBranch, error: msg });
