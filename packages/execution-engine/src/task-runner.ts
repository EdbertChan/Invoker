...
export interface TaskRunnerConfig {
  ...
  /** Structured logger. Falls back to a silent no-op logger when omitted. */
  logger?: Logger;
}

// ── No-op logger (default when none is supplied) ─────────
    ...
    try {
      for (const branch of taskBranches) {
        try {
          ...
          this.logger.error('[rebase] Failed to rebase branch', { branch, baseBranch, error: msg });
