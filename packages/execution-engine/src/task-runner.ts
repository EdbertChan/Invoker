...
import type { Logger, WorkRequest, WorkResponse, ActionType } from '@invoker/contracts';
...
export interface TaskRunnerConfig {
  ...
  /** Structured logger. Defaults to a no-op logger if not provided. */
  logger?: Logger;
}
...
export class TaskRunner {
  ...
  /** @internal */ startPrPolling(taskId: string, reviewId: string, workflowId: string): void {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (status.approved) {
          this.logger.info('[merge-gate] PR approved, completing merge gate', { taskId, reviewId });
  ...
  async checkPrApprovalNow(taskId: string): Promise<void> {
    ...
    try {
      ...
      if (status.approved) {
        this.logger.info('[merge-gate] PR approved (manual check), completing merge gate', { taskId, reviewId });
        this.stopPrPolling(taskId);
        await this.orchestrator.approve(taskId);
      } else if (status.rejected) {
