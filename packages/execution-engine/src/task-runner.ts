...
export interface TaskRunnerConfig {
  ...
  /** Structured logger. Defaults to a no-op logger if unset. */
  logger?: Logger;
}
...
export class TaskRunner {
  ...
  /** @internal */ async ensureRepoMirrorPath(repoUrl: string): Promise<string | undefined> {
    ...
    try {
      ...
      this.logger.warn('[merge] ensureRepoMirrorPath failed', { repoUrl: trimmed, error: String(err) });
      return undefined;
    }
  }
  ...
  async executeTask(task: TaskState): Promise<void> {
    ...
    try {
      ...
      this.logger.error('[TaskRunner] executeTask failed', { taskId: task.id, error: err instanceof Error ? (err.stack ?? err.message) : String(err) });
  ...
  private async executeTaskInner(task: TaskState, attemptId: string): Promise<void> {
    ...
    if (!launchAccepted) {
      this.logger.warn(
        '[TaskRunner] launch rejected as stale/non-executable; killing spawned process',
        { taskId: task.id, attemptId },
      );
      try {
        await executor.kill(handle);
      } catch (killErr) {
        this.logger.warn('[TaskRunner] failed to kill rejected launch', { taskId: task.id, error: String(killErr) });
      }
      await this.cleanupPerTaskDockerExecutor(task);
      return;
    }
      ...
      executor.onComplete(handle, async (response: WorkResponse) => {
        const work = async () => {
          ...
          try {
            ...
            this.logger.error('[TaskRunner] onComplete handler failed', { taskId: task.id, error: err instanceof Error ? (err.stack ?? err.message) : String(err) });
  ...
  async runVisualProofCapture(baseBranch: string, featureBranch: string, slug: string, repoUrl?: string): Promise<string | undefined> {
    try {
      ...
      this.logger.warn('[visual-proof] Capture failed (non-blocking)', { error: err instanceof Error ? err.message : String(err) });
      return undefined;
    }
  }
  ...
  /** @internal */ async removeMergeWorktree(dir: string): Promise<void> {
    try {
      ...
      this.logger.warn('[TaskRunner] removeMergeWorktree failed (best-effort)', { error: err instanceof Error ? err.message : String(err) });
    }
  }
    ...
    for (const task of this.orchestrator.getAllTasks()) {
      if (
        ...
        try {
          ...
          if (status.approved) {
            this.logger.info('[merge-gate] PR approved (refresh), completing merge gate', { reviewId: task.execution.reviewId, taskId: task.id });
            this.stopPrPolling(task.id);
            await this.orchestrator.approve(task.id);
          } else if (status.rejected) {
            this.logger.info('[merge-gate] PR rejected (refresh)', { reviewId: task.execution.reviewId, taskId: task.id, statusText: status.statusText });
            this.stopPrPolling(task.id);
          }
        } catch (err) {
          this.logger.error('[merge-gate] PR status check error', { taskId: task.id, error: err instanceof Error ? (err.stack ?? err.message) : String(err) });
        }
      }
    }
  }
  ...
  /** @internal */ startPrPolling(taskId: string, reviewId: string, workflowId: string): void {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        this.logger.error('[merge-gate] PR poll error', { taskId, error: err instanceof Error ? (err.stack ?? err.message) : String(err) });
  ...
  async checkPrApprovalNow(taskId: string): Promise<void> {
    ...
    try {
      ...
      this.logger.error('[merge-gate] Manual PR check error', { taskId, error: err instanceof Error ? (err.stack ?? err.message) : String(err) });
    }
  }
  ...
  private async cleanupPerTaskDockerExecutor(task: TaskState): Promise<void> {
    ...
    try {
      ...
      this.logger.warn('[TaskRunner] cleanupPerTaskDockerExecutor destroyAll failed', { dockerKey, error: err instanceof Error ? err.message : String(err) });
    }
