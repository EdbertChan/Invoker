...
export interface TaskRunnerConfig {
  ...
  /** Structured logger. Falls back to a silent no-op logger when unset. */
  logger?: Logger;
}
...
export class TaskRunner {
      ...
      executor.onComplete(handle, async (response: WorkResponse) => {
        const work = async () => {
          ...
          try {
            ...
            this.logger.error(`[TaskRunner] onComplete handler failed for task=${task.id}`, { error: err });
  ...
  async runVisualProofCapture(baseBranch: string, featureBranch: string, slug: string, repoUrl?: string): Promise<string | undefined> {
    try {
      ...
      this.logger.warn(`[visual-proof] Capture failed (non-blocking): ${err instanceof Error ? err.message : String(err)}`);
      return undefined;
    }
  }
  ...
  /** @internal */ async removeMergeWorktree(dir: string): Promise<void> {
    try {
      ...
      this.logger.warn(`[TaskRunner] removeMergeWorktree failed (best-effort): ${err instanceof Error ? err.message : String(err)}`);
    }
  }
    ...
    for (const task of this.orchestrator.getAllTasks()) {
      if (
        ...
        try {
          ...
          this.logger.error(`[merge-gate] PR status check error for ${task.id}`, { error: err });
        }
      }
    }
  }
  ...
  /** @internal */ startPrPolling(taskId: string, reviewId: string, workflowId: string): void {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        this.logger.error(`[merge-gate] PR poll error for ${taskId}`, { error: err });
  ...
  async checkPrApprovalNow(taskId: string): Promise<void> {
    ...
    try {
      ...
      this.logger.error(`[merge-gate] Manual PR check error for ${taskId}`, { error: err });
    }
  }
  ...
  private async cleanupPerTaskDockerExecutor(task: TaskState): Promise<void> {
    ...
    try {
      ...
      this.logger.warn(`[TaskRunner] cleanupPerTaskDockerExecutor destroyAll failed for ${dockerKey}: ${err instanceof Error ? err.message : String(err)}`);
    }
    ...
    try {
      for (const branch of taskBranches) {
        try {
          ...
          this.logger.error(`[rebase] Failed to rebase ${branch}: ${msg}`);
