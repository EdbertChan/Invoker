...
export interface TaskRunnerConfig {
  ...
  /** Structured logger. Defaults to a silent no-op if not provided. */
  logger?: Logger;
}
...
export class TaskRunner {
  ...
  /** @internal */ async ensureRepoMirrorPath(repoUrl: string): Promise<string | undefined> {
    ...
    try {
      ...
      this.logger.warn(`[merge] ensureRepoMirrorPath failed for ${trimmed}`, { error: err });
      return undefined;
    }
  }
  ...
  constructor(config: TaskRunnerConfig) {
    ...
    const noop = () => {};
    this.logger = config.logger ?? { debug: noop, info: noop, warn: noop, error: noop, child: () => this.logger };
  }
  ...
  async executeTask(task: TaskState): Promise<void> {
    ...
    try {
      ...
      this.logger.error(`[TaskRunner] executeTask failed for task=${task.id}`, { error: err });
