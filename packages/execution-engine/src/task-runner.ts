...
import { finalizeTaskLaunchFailure } from './task-runner-finalize.js';
...
export class TaskRunner {
  ...
  /** @internal */ pendingPoolSelections = new Map<string, PoolSelection>();
  /** @internal */ freshBaseCommits = new Map<string, FreshBaseCommit>();
  ...
  /** @internal */ completionChain: Promise<void> = Promise.resolve();
  ...
  async executeTask(task: TaskState, dispatchOpts?: LaunchDispatchOptions): Promise<void> {
    ...
    try {
      ...
      await finalizeTaskLaunchFailure(this, {
        task,
        attemptId,
        startGeneration,
        err,
  ...
  private async executeTaskInner(
    ...
    const prepared = await prepareTaskExecution(this, task, attemptId, bench, dispatchOpts);
    if (prepared.kind === 'pivot') return;
    await dispatchTaskExecution(this, task, attemptId, prepared, bench, dispatchOpts);
  }
