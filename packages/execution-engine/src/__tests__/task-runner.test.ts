...
describe('TaskRunner', () => {
  ...
  it('normalizes missing completion lineage through the shared TaskRunner path', async () => {
    const handleWorkerResponse = vi.fn();
    let seenRequest: any;
    let completeCallback: ((response: WorkResponse) => void) | undefined;
    const executorImpl = {
    ...
    const task = makeTask({
      id: 'normalize-lineage',
      status: 'running',
      config: { command: 'echo hi' },
      execution: { generation: 3, selectedAttemptId: 'normalize-lineage-a1' },
    });

    const done = runner.executeTask(task);
    await vi.waitFor(() => expect(seenRequest?.attemptId).toBe('normalize-lineage-a1'));
    completeCallback?.({
      requestId: seenRequest.requestId,
      actionId: task.id,
      status: 'completed',
    ...
    expect(handleWorkerResponse).toHaveBeenCalledWith(
      expect.objectContaining({
        actionId: task.id,
        attemptId: 'normalize-lineage-a1',
