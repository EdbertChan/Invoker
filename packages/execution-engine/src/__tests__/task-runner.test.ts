...
describe('TaskRunner', () => {
  ...
  it('normalizes executor completions that omit attemptId back to the launch attempt', async () => {
    const handleWorkerResponse = vi.fn();
    let seenRequest: any;
    let completeCallback: ((response: WorkResponse) => void) | undefined;
    const executorImpl = {
    ...
    const task = makeTask({
      id: 'legacy-complete-task',
      status: 'running',
      config: { command: 'echo hi' },
      execution: { generation: 3, selectedAttemptId: 'legacy-complete-task-a1' },
    });

    const done = runner.executeTask(task);
    await vi.waitFor(() => expect(seenRequest?.attemptId).toBe('legacy-complete-task-a1'));
    completeCallback?.({
      requestId: seenRequest.requestId,
      actionId: task.id,
      executionGeneration: seenRequest.executionGeneration,
    ...
    expect(handleWorkerResponse).toHaveBeenCalledWith(
      expect.objectContaining({
        actionId: task.id,
        attemptId: 'legacy-complete-task-a1',
