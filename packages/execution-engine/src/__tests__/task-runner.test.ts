  ...
  describe('buildMergeSummary', () => {
    ...
    it('includes Test Plan with command task results', async () => {
      const tasks = [
        makeTask({
          ...
          status: 'completed',
          config: { workflowId: 'wf-1', command: 'pnpm test' },
          execution: { branch: 'experiment/verify-pass', exitCode: 0 },
        }),
        makeTask({
          id: 'verify-fail',
          ...
          execution: { branch: 'experiment/verify-fail', exitCode: 1 },
        }),
      ];
      const { executor } = createExecutorForSummary(tasks, { name: 'Workflow' });
      (executor as any).gitDiffStat = vi.fn();
    ...
    it('omits Test Plan when no command tasks', async () => {
      const tasks = [
        makeTask({
          id: 'prompt-1',
          description: 'Prompt-only task one',
          status: 'completed',
          config: { workflowId: 'wf-1' },
        ...
        makeTask({
          id: 'prompt-2',
          description: 'Prompt-only task two',
          status: 'completed',
          config: { workflowId: 'wf-1' },
    ...
    it('sanitizes task IDs with special chars in Mermaid', async () => {
      const tasks = [
        ...
        makeTask({
          ...
          description: 'Task C',
          status: 'completed',
          dependencies: ['taskA', 'wf-123/my-task'],
          config: { workflowId: 'wf-1' },
          execution: { branch: 'experiment/taskC' },
        }),
      ];
