...
describe('TaskRunner', () => {
  ...
  it('normalizes completion responses to the INV-113 launch lineage', async () => {
    const handleWorkerResponse = vi.fn();
    let seenRequest: any;
    let completeCallback: ((response: WorkResponse) => void) | undefined;
    const executorImpl = {
    ...
    const task = makeTask({
      id: 'lineage-normalized-task',
      status: 'running',
      config: { command: 'echo hi' },
      execution: { generation: 4, selectedAttemptId: 'lineage-normalized-task-a1' },
    });
    ...
    expect(handleWorkerResponse).toHaveBeenCalledWith(
      expect.objectContaining({
        actionId: task.id,
        attemptId: 'lineage-normalized-task-a1',
