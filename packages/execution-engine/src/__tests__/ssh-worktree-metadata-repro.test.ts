  ...
  it('does not attach stale SSH startup failure metadata after the selected attempt advances', async () => {
    ...
    const currentAttempt = makeTask({
      ...
      execution: {
        ...
        branch: 'experiment/wf-1/test-execution-engine-live-attempt-2',
      },
    });
    ...
    expect(logEvent).toHaveBeenCalledWith('wf-1/test-execution-engine', 'task.executor.startup-failure.stale', expect.objectContaining({
      attemptId: 'attempt-1',
      currentSelectedAttemptId: 'attempt-2',
      workspacePath: ownerPath,
      branch,
    }));
  });
