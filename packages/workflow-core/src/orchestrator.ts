...
import {
  autoStartExternallyUnblockedReadyTasks as scheduleAutoStartExternallyUnblockedReadyTasks,
  autoStartReadyTasks as scheduleAutoStartReadyTasks,
  autoStartUnblockedTasks as scheduleAutoStartUnblockedTasks,
  drainScheduler as scheduleDrainScheduler,
  enqueueIfNotScheduled as scheduleEnqueueIfNotScheduled,
  type OrchestratorSchedulingHost,
} from './orchestrator/scheduling.js';
import {
  rejectApproval as transitionRejectApproval,
  resumeTaskAfterFixApproval as transitionResumeTaskAfterFixApproval,
  setFixAwaitingApproval as transitionSetFixAwaitingApproval,
  setTaskApprovalStatus as transitionSetTaskApprovalStatus,
  type OrchestratorTransitionHost,
} from './orchestrator/transitions.js';
import {
  buildRemoveDelta as createRemoveDelta,
  buildUpdateDelta as createUpdateDelta,
  publishTaskDelta,
} from './orchestrator/events.js';
import {
  descriptionForMergeNode,
  editTaskMergeMode as mergeEditTaskMergeMode,
  mergeTrace,
  type MergeMode,
  type OrchestratorMergeHost,
} from './orchestrator/merge.js';
import {
  selectExperiment as experimentSelectExperiment,
  selectExperiments as experimentSelectExperiments,
  type OrchestratorExperimentHost,
...
export class Orchestrator {
  ...
  private schedulingHost(): OrchestratorSchedulingHost {
    return {
  ...
  private transitionHost(): OrchestratorTransitionHost {
    return {
      persistence: this.persistence,
      messageBus: this.messageBus,
      logger: this.logger,
      ...
      taskNotFoundError: (taskId) => new OrchestratorError(OrchestratorErrorCode.TASK_NOT_FOUND, `Task ${taskId} not found`),
      traceMerge: (tag, data) => mergeTrace(tag, data),
    };
  }

  private experimentHost(): OrchestratorExperimentHost {
    return {
      persistence: this.persistence,
      messageBus: this.messageBus,
      logger: this.logger,
  ...
  private mergeHost(): OrchestratorMergeHost {
    return {
      persistence: this.persistence,
      refreshFromDb: () => this.refreshFromDb(),
      stateGetTask: (taskId) => this.stateGetTask(taskId),
  ...
  private buildUpdateDelta(before: TaskState, after: TaskState, changes: TaskStateChanges): TaskDelta {
    return createUpdateDelta(before, after, changes);
  }
  ...
  private resetSubgraphToPending(
    ...
    for (const delta of pendingTaskDeltas) {
      publishTaskDelta(this.messageBus, delta);
    }
  ...
  private cancelActiveBeforeInvalidation(
    ...
    for (const t of candidates) {
      ...
      publishTaskDelta(this.messageBus, this.buildUpdateDelta(t, updated, changes));
  ...
  prepareTaskForNewAttempt(taskId: string, reason: string): TaskState {
    ...
    publishTaskDelta(this.messageBus, this.buildUpdateDelta(task, updated, changes));
    return updated;
  }
  ...
  private setTaskApprovalStatus(
    ...
    transitionSetTaskApprovalStatus(this.transitionHost(), taskId, status, eventName, additionalChanges);
  }
  ...
  setFixAwaitingApproval(taskId: string, originalError: string): void {
    transitionSetFixAwaitingApproval(this.transitionHost(), taskId, originalError);
  }
  ...
  async resumeTaskAfterFixApproval(taskId: string): Promise<TaskState[]> {
    return transitionResumeTaskAfterFixApproval(this.transitionHost(), taskId);
  }
  ...
  reject(taskId: string, reason?: string): void {
    transitionRejectApproval(this.transitionHost(), taskId, reason);
  }
  ...
  selectExperiment(taskId: string, experimentId: string): TaskState[] {
    return experimentSelectExperiment(this.experimentHost(), taskId, experimentId);
  }
  ...
  selectExperiments(
    ...
    return experimentSelectExperiments(
      this.experimentHost(),
      taskId,
      experimentIds,
      combinedBranch,
    ...
    if (isReady) {
      ...
      if (current) {
        ...
        if (blocker !== undefined) {
          ...
          publishTaskDelta(this.messageBus, blockedDelta);
  ...
  recreateTask(taskId: string): TaskState[] {
    ...
    for (const id of toResetIds) {
      ...
      this.persistence.logEvent?.(id, 'task.pending', changesWithGeneration);
      publishTaskDelta(this.messageBus, this.buildUpdateDelta(current, recreateUpdated, changesWithGeneration));
  ...
  editTaskCommand(taskId: string, newCommand: string): TaskState[] {
    ...
    publishTaskDelta(this.messageBus, cmdDelta);
    ...
    publishTaskDelta(this.messageBus, promptDelta);
    ...
    publishTaskDelta(this.messageBus, typeDelta);
    ...
    publishTaskDelta(this.messageBus, poolDelta);
    ...
    publishTaskDelta(this.messageBus, agentDelta);
  ...
  editTaskMergeMode(
    ...
    return mergeEditTaskMergeMode(this.mergeHost(), taskId, mergeMode);
  }
  ...
  editTaskFixContext(
    ...
    publishTaskDelta(this.messageBus, fixContextDelta);
  ...
  setTaskExternalGatePolicies(taskId: string, updates: ExternalGatePolicyUpdate[]): TaskState[] {
    ...
    publishTaskDelta(this.messageBus, policyDelta);
  ...
  forkWorkflow(workflowId: string, opts?: { autoStart?: boolean }): ForkWorkflowResult {
    ...
    for (const src of sourceNonMerge) {
      ...
      publishTaskDelta(this.messageBus, { type: 'created', task: newTask });
    ...
    publishTaskDelta(this.messageBus, { type: 'created', task: newMerge });
  ...
  private replaceTaskInPlace(
    ...
    for (const id of descendantIds) {
      ...
      publishTaskDelta(this.messageBus, this.buildUpdateDelta(staleBefore, staleUpdated, staleChanges));
    ...
    publishTaskDelta(this.messageBus, this.buildUpdateDelta(sourceBefore, sourceUpdated, sourceChanges));
  ...
  deleteWorkflow(workflowId: string): void {
    ...
    for (const task of affectedTasks) {
      publishTaskDelta(this.messageBus, this.buildRemoveDelta(task));
    }
  }
  ...
  cancelWorkflow(workflowId: string): { cancelled: string[]; runningCancelled: string[] } {
    ...
    for (const task of allTasks) {
      ...
      this.persistence.logEvent?.(id, 'task.cancelled', changes);
      publishTaskDelta(this.messageBus, this.buildUpdateDelta(task, wfCancelUpdated, changes));
      cancelled.push(id);
    }
  ...
  private autoStartReadyTasks(taskIds: string[], priority: number = 0): TaskState[] {
    return scheduleAutoStartReadyTasks(this.schedulingHost(), taskIds, priority);
  }

  private enqueueIfNotScheduled(taskId: string, priority: number = 0): void {
    scheduleEnqueueIfNotScheduled(this.schedulingHost(), taskId, priority);
  }
  ...
  autoStartExternallyUnblockedReadyTasks(): TaskState[] {
    return scheduleAutoStartExternallyUnblockedReadyTasks(this.schedulingHost());
  }

  private autoStartUnblockedTasks(): TaskState[] {
    return scheduleAutoStartUnblockedTasks(this.schedulingHost());
  }
  ...
  private drainScheduler(): TaskState[] {
    return scheduleDrainScheduler(this.schedulingHost());
  }
  ...
  markTaskRunningAfterLaunch(taskId: string, attemptId: string, launchedAt: Date = new Date()): boolean {
    ...
    if (task.status !== 'fixing_with_ai') {
      ...
      publishTaskDelta(this.messageBus, this.buildUpdateDelta(task, launchUpdated, changes));
