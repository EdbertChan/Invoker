...
import {
  ...
  clearQueuedSchedulerEntries as clearQueuedSchedulerEntriesImpl,
...
export class Orchestrator {
  ...
  private schedulerHost(): SchedulerDomainHost {
    return {
      ...
      replaceSelectedAttempt: (task, patch) => this.replaceSelectedAttempt(task, patch),
      getReadyTasks: () => this.stateMachine.getReadyTasks(),
      getAllTasks: () => this.stateMachine.getAllTasks(),
      updateAttempt: (attemptId, changes) => this.taskRepository.updateAttempt(attemptId, changes),
      claimAttemptForLaunch: this.taskRepository.claimAttemptForLaunch
        ? (attemptId, changes, now) => this.taskRepository.claimAttemptForLaunch!(attemptId, changes, now)
        : undefined,
    };
  }
  ...
  private transitionHost(): TransitionDomainHost {
    return {
      ...
      buildUpdateDelta: (before, after, changes) => this.buildUpdateDelta(before, after, changes),
      updateAttempt: (attemptId, changes) => this.taskRepository.updateAttempt(attemptId, changes),
      failTaskAndAttempt: (taskId, taskChanges, attemptPatch) =>
        this.taskRepository.failTaskAndAttempt(taskId, taskChanges, attemptPatch),
  ...
  private taskDeltaEventHost(): TaskDeltaEventHost {
    return {
      persistence: this.persistence,
      messageBus: this.messageBus,
      buildUpdateDelta: (before, after, changes) => this.buildUpdateDelta(before, after, changes),
  ...
  private experimentHost(): ExperimentDomainHost {
    return {
      ...this.taskDeltaEventHost(),
      logger: this.logger,
      refreshFromDb: () => this.refreshFromDb(),
      stateGetTask: (taskId) => this.stateGetTask(taskId),
      getAllTasks: () => this.stateMachine.getAllTasks(),
      findNewlyReadyTasks: (taskId) => this.stateMachine.findNewlyReadyTasks(taskId),
      isActiveForInvalidationStatus: (status) => isActiveForInvalidation(status),
  ...
  private clearQueuedSchedulerEntries(taskId: string, attemptId?: string): void {
    clearQueuedSchedulerEntriesImpl(this.schedulerHost(), taskId, attemptId);
  }
  ...
  handleWorkerResponse(response: WorkResponse): TaskState[] {
    ...
    switch (parsed.type) {
      ...
      case 'select_experiment':
        return applyParsedWorkerTransition(this.transitionHost(), canonicalTaskId, parsed);
      default:
        return [];
    }
  }
  ...
  selectExperiment(taskId: string, experimentId: string): TaskState[] {
    return selectExperimentImpl(this.experimentHost(), taskId, experimentId);
  }
  ...
  private handleCompleted(
    ...
    return applyParsedWorkerTransition(this.transitionHost(), taskId, parsed);
  }
  ...
  private finalizeFailedTask(
    ...
    return finalizeFailedTransition(this.transitionHost(), taskId, executionFields, eventName);
  }
  ...
  private autoStartReadyTasks(taskIds: string[], priority: number = 0): TaskState[] {
    return autoStartReadyTasksImpl(this.schedulerHost(), taskIds, priority);
  }

  private enqueueIfNotScheduled(taskId: string, priority: number = 0): void {
    enqueueIfNotScheduledImpl(this.schedulerHost(), taskId, priority);
  }
  ...
  autoStartExternallyUnblockedReadyTasks(): TaskState[] {
    return autoStartExternallyUnblockedReadyTasksImpl(this.schedulerHost());
  }

  private autoStartUnblockedTasks(): TaskState[] {
    return autoStartUnblockedTasksImpl(this.schedulerHost());
  }
  ...
  private drainScheduler(): TaskState[] {
    return drainSchedulerImpl(this.schedulerHost());
  }

  private drainDeferredTasks(): TaskState[] {
    if (this.deferredTaskIds.size === 0) return [];
