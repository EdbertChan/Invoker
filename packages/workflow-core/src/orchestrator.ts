...
import {
  setTaskApprovalStatus as setTaskApprovalStatusImpl,
} from './orchestrator/transition-domain.js';
import {
  publishTaskDelta,
  publishTaskDeltas,
} from './orchestrator/events-domain.js';
import {
  selectExperiment as selectExperimentImpl,
  selectExperiments as selectExperimentsImpl,
} from './orchestrator/experiment-domain.js';
import {
...
export class Orchestrator {
  ...
  startExecution(): TaskState[] {
    return startExecutionImpl(this as unknown as Parameters<typeof startExecutionImpl>[0]);
  }
  ...
  private setTaskApprovalStatus(
    ...
    setTaskApprovalStatusImpl(
      this as unknown as Parameters<typeof setTaskApprovalStatusImpl>[0],
      taskId,
      status,
      eventName,
      additionalChanges,
  ...
  selectExperiment(taskId: string, experimentId: string): TaskState[] {
    return selectExperimentImpl(
      this as unknown as Parameters<typeof selectExperimentImpl>[0],
      taskId,
      experimentId,
    );
  }
  ...
  selectExperiments(
    ...
    return selectExperimentsImpl(
      this as unknown as Parameters<typeof selectExperimentsImpl>[0],
      taskId,
      experimentIds,
      combinedBranch,
      combinedCommit,
  ...
  editTaskMergeMode(
    ...
    return editTaskMergeModeImpl(this as unknown as Parameters<typeof editTaskMergeModeImpl>[0], taskId, mergeMode, {
      taskNotFound: (id) => new OrchestratorError(OrchestratorErrorCode.TASK_NOT_FOUND, `Task ${id} not found`),
      dispatchPostMutation: (action, id) => this.dispatchPostMutation(action, id),
    });
  }
  ...
  private mergeTrace(tag: string, data: Record<string, unknown>): void {
    mergeTrace(tag, data);
  }

  private autoStartReadyTasks(taskIds: string[], priority: number = 0): TaskState[] {
    return autoStartReadyTasksImpl(
      this as unknown as Parameters<typeof autoStartReadyTasksImpl>[0],
      taskIds,
      priority,
    );
  }
  ...
  private enqueueIfNotScheduled(taskId: string, priority: number = 0): void {
    enqueueIfNotScheduledImpl(
      this as unknown as Parameters<typeof enqueueIfNotScheduledImpl>[0],
      taskId,
      priority,
    );
  }
  ...
  autoStartExternallyUnblockedReadyTasks(): TaskState[] {
    return autoStartExternallyUnblockedReadyTasksImpl(
      this as unknown as Parameters<typeof autoStartExternallyUnblockedReadyTasksImpl>[0],
    );
  }

  private autoStartUnblockedTasks(): TaskState[] {
    return autoStartUnblockedTasksImpl(
      this as unknown as Parameters<typeof autoStartUnblockedTasksImpl>[0],
    );
  }
  ...
  private drainScheduler(): TaskState[] {
    return drainSchedulerImpl(this as unknown as Parameters<typeof drainSchedulerImpl>[0]);
  }
  ...
  markTaskRunningAfterLaunch(taskId: string, attemptId: string, launchedAt: Date = new Date()): boolean {
    return markTaskRunningAfterLaunchImpl(
      this as unknown as Parameters<typeof markTaskRunningAfterLaunchImpl>[0],
      taskId,
      attemptId,
      launchedAt,
    );
