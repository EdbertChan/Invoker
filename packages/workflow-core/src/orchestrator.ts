...
export class Orchestrator {
  ...
  private writeAndSync(
    ...
    if (process.env.NODE_ENV !== 'test' && TRACE_PERSIST_SYNC) {
      ...
      this.logger.info('[persist-sync]', {
        ...
        pendingFix: ex.pendingFixError ? true : false,
        inputPrompt: ex.inputPrompt ? true : false,
  ...
  private setTaskApprovalStatus(
    ...
    if (task.config.isMergeNode && changes.execution && 'workspacePath' in changes.execution) {
      ...
      this.logger.info(
        ...
        { mergeNode: id, workspacePath: changes.execution.workspacePath ?? 'NULL' },
      );
    }
    this.writeAndSync(id, changes);
  ...
  setFixAwaitingApproval(taskId: string, originalError: string): void {
    ...
    this.logger.info('[setFixAwaitingApproval] delta.changes.execution', {
      taskId: tid,
      execution: changes.execution ? JSON.stringify(changes.execution) : undefined,
    });
  ...
  async approve(taskId: string): Promise<TaskState[]> {
    ...
    if (!task || !isApprovalState) {
      ...
      this.logger.info('[orchestrator.approve] skipped', {
        taskId,
        reason: !task ? 'task_not_found' : 'invalid_status',
        status: task?.status,
        expectedStatus: 'awaiting_approval|review_ready',
      });
      return [];
    }
  ...
  selectExperiment(taskId: string, experimentId: string): TaskState[] {
    ...
    this.logger.info('[orchestrator] selectExperiment', {
      taskId: reconId,
      newlyReady: readyTaskIds.length,
      readyTaskIds,
    });
    const started = this.autoStartReadyTasks(readyTaskIds);
    this.checkWorkflowCompletion();
  ...
  restartTask(taskId: string): TaskState[] {
    ...
    if (task.config.isMergeNode) {
      this.logger.info('[merge-gate-workspace] restartTask', {
        mergeNode: id,
        workspacePathBefore: task.execution.workspacePath ?? 'none',
    ...
    if (afterRt.config.isMergeNode) {
      this.logger.info('[merge-gate-workspace] restartTask after reset', {
        mergeNode: id,
        workspacePathAfter: afterRt.execution.workspacePath ?? 'none',
      });
  ...
  retryWorkflow(workflowId: string): TaskState[] {
    ...
    this.logger.info('[orchestrator] retryWorkflow reset', {
      workflowId,
      resetTasks: affectedIds.length,
      totalTasks: allTasks.length,
      roots: retryRootIds.length,
  ...
  recreateWorkflow(workflowId: string): TaskState[] {
    ...
    this.logger.info('[agent-session-trace] recreateWorkflow resetChanges.execution', {
      clearsAgentSessionId: true,
      clearsContainerId: true,
      nextRunDbValue: 'NULL',
    });
