...
import {
  drainScheduler as drainSchedulerDomain,
  markTaskRunningAfterLaunch as markTaskRunningAfterLaunchDomain,
  type SchedulerDomainDeps,
} from './orchestrator/scheduler-domain.js';
import {
  finalizeFailedTaskTransition,
  handleCompletedTransition,
  handleFailedTransition,
  handleNeedsInputTransition,
  handleReviewReadyTransition,
  type TransitionDomainDeps,
} from './orchestrator/transitions-domain.js';
import {
  buildRemoveTaskDelta,
  buildUpdateTaskDelta,
  publishTaskDelta as publishTaskDeltaDomain,
  publishTaskDeltas as publishTaskDeltasDomain,
} from './orchestrator/events-domain.js';
import {
  assertMergeInvariants,
  buildMergeTask,
  descriptionForMergeNode as descriptionForMergeNodeDomain,
  getMergeNode as getMergeNodeDomain,
  mergeTrace,
...
export function descriptionForMergeNode(plan: Pick<PlanDefinition, 'name' | 'onFinish' | 'mergeMode'>): string {
  return descriptionForMergeNodeDomain(plan);
}
...
export class Orchestrator {
  ...
  private buildRemoveDelta(task: TaskState): TaskDelta {
    return buildRemoveTaskDelta(task);
  }

  private publishTaskDelta(delta: TaskDelta): void {
    publishTaskDeltaDomain(this.messageBus, delta);
  }

  private publishTaskDeltas(deltas: readonly TaskDelta[]): void {
    publishTaskDeltasDomain(this.messageBus, deltas);
  }
  ...
  private schedulerDomainDeps(): SchedulerDomainDeps {
    return {
      ...
      getTaskLaunchReadiness: (taskId, opts) => this.getTaskLaunchReadiness(taskId, opts),
      ...
      buildAndPublishUpdateDelta: (before, after, changes) => {
        this.publishTaskDelta(this.buildUpdateDelta(before, after, changes));
      },
      clearQueuedSchedulerEntries: (taskId, attemptId) => this.clearQueuedSchedulerEntries(taskId, attemptId),
    };
  }

  private transitionDomainDeps(): TransitionDomainDeps {
    return {
      persistence: this.persistence,
      taskRepository: this.taskRepository,
      scheduler: this.scheduler,
      ...
      taskNotFoundError: (context, taskId) =>
        new OrchestratorError(OrchestratorErrorCode.TASK_NOT_FOUND, `${context}: task ${taskId} not found in graph`),
    };
  }

  private experimentDomainDeps(): ExperimentDomainDeps {
    return {
      logger: this.logger,
      persistence: this.persistence,
      stateGetTask: (taskId) => this.stateGetTask(taskId),
  ...
  loadPlan(plan: PlanDefinition, opts?: { allowGraphMutation?: boolean }): void {
    ...
    const mergeTask = buildMergeTask(plan, workflowId, leafIds);
  ...
  selectExperiment(taskId: string, experimentId: string): TaskState[] {
    this.refreshFromDb();
    return selectExperimentDomain(this.experimentDomainDeps(), taskId, experimentId);
  }
  ...
  editTaskCommand(taskId: string, newCommand: string): TaskState[] {
    ...
    this.publishTaskDelta(modelDelta);
  ...
  getMergeNode(workflowId: string): TaskState | undefined {
    return getMergeNodeDomain(this.stateMachine.getAllTasks(), workflowId);
  }
  ...
  private handleCompleted(
    ...
    return handleCompletedTransition(this.transitionDomainDeps(), taskId, parsed);
  }
  ...
  private finalizeFailedTask(
    ...
    return finalizeFailedTaskTransition(this.transitionDomainDeps(), taskId, executionFields, eventName);
  }
  ...
  private handleReviewReady(
    ...
    return handleReviewReadyTransition(this.transitionDomainDeps(), taskId, parsed);
  }
  ...
  private handleFailed(
    ...
    return handleFailedTransition(this.transitionDomainDeps(), taskId, parsed);
  }
  ...
  private handleNeedsInput(
    ...
    return handleNeedsInputTransition(this.transitionDomainDeps(), taskId, parsed);
  }
  ...
  private handleSpawnExperiments(
    ...
    return handleSpawnExperimentsDomain(this.experimentDomainDeps(), taskId, parsed);
  }
  ...
  private checkExperimentCompletion(taskId: string): void {
    checkExperimentCompletionDomain(this.experimentDomainDeps(), taskId);
  }
  ...
  private drainScheduler(): TaskState[] {
    return drainSchedulerDomain(this.schedulerDomainDeps());
  }
  ...
  markTaskRunningAfterLaunch(taskId: string, attemptId: string, launchedAt: Date = new Date()): boolean {
    return markTaskRunningAfterLaunchDomain(this.schedulerDomainDeps(), taskId, attemptId, launchedAt);
  }
}
