...
export class Orchestrator {
  ...
  private writeAndSync(
    ...
    if (process.env.NODE_ENV !== 'test' && TRACE_PERSIST_SYNC) {
      ...
      this.logger.info('[persist-sync] writeAndSync', {
        taskId: id,
  ...
  handleWorkerResponse(response: WorkResponse): TaskState[] {
    ...
    // and should not affect the graph.
    {
      ...
      if (earlyTask) {
        ...
        if (response.attemptId) {
          if (!activeAttemptId || response.attemptId !== activeAttemptId) {
            this.logger.warn('[worker-response] stale attempt rejected', {
              taskId: earlyTask.id,
        ...
        if (
          ...
          this.logger.warn('[worker-response] stale generation rejected', {
            taskId: earlyTask.id,
      ...
      if (earlyTask) {
        ...
        if (!executableStatuses.has(earlyTask.status)) {
          this.logger.warn('[orchestrator] handleWorkerResponse ignored non-executable task response', {
    ...
    if (!('type' in parsed)) {
      ...
      if (!task) {
        this.logger.warn('[worker-response] protocol failure unknown task', {
      ...
      this.logger.warn('[worker-response] protocol failure', {
    ...
    if (!task) {
      this.logger.warn('[worker-response] task not in graph', { taskId });
      return [];
    }
  ...
  setFixAwaitingApproval(taskId: string, originalError: string): void {
    ...
    this.logger.info('[setFixAwaitingApproval] execution changes', {
      taskId: tid,
  ...
  async approve(taskId: string): Promise<TaskState[]> {
    ...
    if (!task || !isApprovalState) {
      ...
      this.logger.info('[orchestrator.approve] skipped', {
        ...
        found: !!task,
        status: task?.status,
        expectedStatuses: ['awaiting_approval', 'review_ready'],
      });
      return [];
    }
  ...
  restartTask(taskId: string): TaskState[] {
    ...
    this.logger.info('[orchestrator] restartTask', { taskId: id, previousStatus: prevStatus });
    if (task.config.isMergeNode) {
      this.logger.info('[merge-gate-workspace] restartTask before reset', {
        mergeNode: id,
        workspacePath: task.execution.workspacePath ?? 'none',
        clearsWorkspacePath: false,
      });
    ...
    this.logger.info('[agent-session-trace] restartTask before writeAndSync', {
      taskId: id,
      agentSessionId: t0.execution.agentSessionId ?? null,
      resetClearsAgentSessionId: true,
      resetClearsContainerId: true,
      branchUnchanged: true,
      workspacePathUnchanged: true,
    });
    const { affectedIds } = this.resetSubgraphToPending([id], resetChanges, {
      forceResetIds: new Set([id]),
    });
    const afterRt = this.stateGetTask(id)!;
    this.logger.info('[agent-session-trace] restartTask after writeAndSync', {
      taskId: id,
      agentSessionId: afterRt.execution.agentSessionId ?? null,
    });
    if (afterRt.config.isMergeNode) {
      this.logger.info('[merge-gate-workspace] restartTask after reset', {
        mergeNode: id,
    ...
    if (affectedIds.length > 1) {
      this.logger.info('[orchestrator] restartTask invalidated downstream tasks', {
        taskId: id,
    ...
    this.logger.info('[orchestrator] restartTask ready check', { taskId: id, ready: isReady });
    if (isReady) {
  ...
  retryWorkflow(workflowId: string): TaskState[] {
    ...
    this.logger.info('[orchestrator] retryWorkflow reset summary', {
      ...
      preservedCompletedOutsideInvalidatedSubgraphs: true,
    });
  ...
  recreateTask(taskId: string): TaskState[] {
    ...
    this.logger.info('[orchestrator] recreateTask', {
      rootTaskId: rootId,
  ...
  recreateWorkflow(workflowId: string): TaskState[] {
    ...
    this.logger.info('[orchestrator] recreateWorkflow', {
      workflowId,
      resetCount: allTasks.length,
    });
    this.logger.info('[agent-session-trace] recreateWorkflow reset changes', {
      clearsAgentSessionId: true,
      clearsContainerId: true,
    });
    for (const task of allTasks) {
      const prevSess = task.execution.agentSessionId ?? null;
      const prevCt = task.execution.containerId ?? null;
      if (task.config.isMergeNode) {
        this.logger.info('[merge-gate-workspace] recreateWorkflow clearing workspace path', {
          mergeNode: task.id,
  ...
  private handleSpawnExperiments(
    ...
    if (!wfId) {
      this.logger.warn('[orchestrator] handleSpawnExperiments skipped missing workflowId', {
        taskId,
      });
      return [];
    }
  ...
  private autoStartReadyTasks(taskIds: string[], priority: number = 0): TaskState[] {
    for (const taskId of taskIds) {
      ...
      if (task.status === 'blocked') {
        this.logger.info('[orchestrator] autoStartReadyTasks unblocking task', {
          taskId,
  ...
  private drainScheduler(): TaskState[] {
    ...
    while (job && availableSlots > 0) {
      ...
      if (!task || task.status !== 'pending') {
        this.logger.info('[orchestrator] drainScheduler skipping task', {
          taskId: job.taskId,
          reason: 'not_pending',
        });
      ...
      this.logger.info('[orchestrator] drainScheduler started task', {
        taskId: job.taskId,
        attemptId,
        phase: 'launching',
  ...
  markTaskRunningAfterLaunch(taskId: string, attemptId: string, launchedAt: Date = new Date()): boolean {
    ...
    const task = this.stateGetTask(taskId);
    if (!task) {
      this.logger.info('[orchestrator] markTaskRunningAfterLaunch rejected', {
        taskId,
        attemptId,
        reason: 'not_found',
      });
