...
export class Orchestrator {
  ...
  /**
   * Select a winning experiment for a reconciliation task — **recreate-class**
   ...
   * (`MUTATION_POLICIES.selectedExperiment` → `recreateTask` / task scope).
   *
   * Why recreate-class. The INV-55 experiment brief classifies single
   * experiment selection as a downstream-input mutation: the reconciliation
   * task's result (its selected branch/commit) is the execution input that
   * downstream consumers use, so changing the winner invalidates downstream
   * attempts with fresh-lineage semantics while preserving the reconciliation
   * task identity.
   *
   ...
   *      `MUTATION_POLICIES.selectedExperiment.action`. Today the policy
   *      resolves to `recreateTask`, so the consumer subgraph is reset with
   *      fresh-lineage semantics and each affected task's execution
   *      generation is bumped exactly once. For the initial-selection path
   *      no downstream reset is needed because nothing has executed yet
   *      against the recon's result.
   ...
   * one.
   *
   * The re-selection reset deliberately consumes the policy table instead
   * of hard-coding the primitive at the call site; the experiment brief for
   * INV-55 selected table-driven recreate-class invalidation for
   * `selectedExperiment` and `selectedExperimentSet`.
   */
  selectExperiment(taskId: string, experimentId: string): TaskState[] {
    ...
    if (isReSelection) {
      ...
      for (const dsId of directDownstream) {
        this.dispatchPostMutation(MUTATION_POLICIES.selectedExperiment.action, dsId);
      }
    }
    ...
    if (isReSelection) {
      ...
      for (const dsId of directDownstreamAfter) {
        if (this.stateGetTask(dsId)) {
          this.dispatchPostMutation(MUTATION_POLICIES.selectedExperimentSet.action, dsId);
        }
      }
    }
