...
export class Orchestrator {
  ...
  setFixAwaitingApproval(taskId: string, originalError: string): void {
    ...
    this.logger.info('[setFixAwaitingApproval] delta.changes.execution', {
      taskId: tid,
      changesExecution: changes.execution,
    });
  ...
  restartTask(taskId: string): TaskState[] {
    ...
    this.logger.info('[orchestrator] restartTask', { taskId: id, prevStatus });
    if (task.config.isMergeNode) {
    ...
    if (affectedIds.length > 1) {
      this.logger.info('[orchestrator] restartTask downstream invalidation', {
        taskId: id,
        invalidatedDownstreamTaskCount: affectedIds.length - 1,
      });
    }

    const readyTasks = this.stateMachine.getReadyTasks();
    const isReady = readyTasks.some((t) => t.id === id);
    this.logger.info('[orchestrator] restartTask readiness', { taskId: id, ready: isReady });
    if (isReady) {
  ...
  retryWorkflow(workflowId: string): TaskState[] {
    ...
    this.logger.info('[orchestrator] retryWorkflow reset', {
      workflowId,
      resetTaskCount: affectedIds.length,
  ...
  private handleSpawnExperiments(
    ...
    if (!wfId) {
      this.logger.warn('[orchestrator] handleSpawnExperiments: task has no workflowId; skipping', {
        taskId,
      });
      return [];
    }
  ...
  private drainScheduler(): TaskState[] {
    ...
    while (job && availableSlots > 0) {
      ...
      if (!task || task.status !== 'pending') {
        this.logger.info('[orchestrator] drainScheduler: skipping task not pending', {
          taskId: job.taskId,
        });
