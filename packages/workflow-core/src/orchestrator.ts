...
export class Orchestrator {
  ...
  handleWorkerResponse(response: WorkResponse): TaskState[] {
    ...
    if (!task) {
      this.logger.warn(`[worker-response] task not in graph taskId=${taskId} (stale response?)`);
      return [];
    }
  ...
  retryTask(taskId: string): TaskState[] {
    ...
    this.logger.info(`[orchestrator] retryTask "${id}" (was ${prevStatus})`);
    if (task.config.isMergeNode) {
      this.logger.info(
    ...
    this.logger.info(`[orchestrator] retryTask "${id}": ready=${isReady}`);
    if (isReady) {
