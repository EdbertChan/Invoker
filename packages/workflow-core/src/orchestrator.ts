...
export class Orchestrator {
  ...
  restartTask(taskId: string): TaskState[] {
    this.logger.warn('[orchestrator] restartTask is deprecated, routing to recreateTask', {
      taskId,
      hint: 'Use retryTask() for lineage-preserving reset or recreateTask() for fresh-lineage reset explicitly.',
    });
  ...
  retryTask(taskId: string): TaskState[] {
    ...
    this.logger.info('[orchestrator] retryTask', { taskId: id, prevStatus });
    if (task.config.isMergeNode) {
  ...
  recreateTask(taskId: string): TaskState[] {
    ...
    this.logger.info('[orchestrator] recreateTask: resetting tasks', { rootId, resetCount: toResetIds.length });
  ...
  private autoStartReadyTasks(taskIds: string[], priority: number = 0): TaskState[] {
    for (const taskId of taskIds) {
      ...
      if (task.status === 'blocked') {
        this.logger.info('[orchestrator] autoStartReadyTasks: unblocking task', { taskId, reason: 'deps now satisfied' });
  ...
  private drainScheduler(): TaskState[] {
    ...
    while (job && availableSlots > 0) {
      ...
      this.logger.info('[orchestrator] drainScheduler: dequeued', { taskId: job.taskId, status: task?.status ?? 'NOT_FOUND' });
