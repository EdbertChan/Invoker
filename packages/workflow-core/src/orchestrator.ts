...
import {
  TASK_DELTA_CHANNEL,
  buildUpdateTaskDelta,
  publishTaskDelta as publishTaskDeltaForHost,
  publishTaskRemoval as publishTaskRemovalForHost,
  publishTaskUpdate as publishTaskUpdateForHost,
  type TaskEventHost,
} from './orchestrator/events.js';
import {
  autoStartExternallyUnblockedReadyTasks as autoStartExternallyUnblockedReadyTasksForHost,
  autoStartReadyTasks as autoStartReadyTasksForHost,
  autoStartUnblockedTasks as autoStartUnblockedTasksForHost,
  clearQueuedSchedulerEntries as clearQueuedSchedulerEntriesForHost,
  drainScheduler as drainSchedulerForHost,
  getQueueStatus as getQueueStatusForHost,
  getTaskLaunchReadiness as getTaskLaunchReadinessForHost,
  startExecution as startExecutionForHost,
  type SchedulerDomainHost,
} from './orchestrator/scheduling.js';
import {
  deferTask as deferTaskForHost,
  markTaskRunningAfterLaunch as markTaskRunningAfterLaunchForHost,
  type TransitionDomainHost,
} from './orchestrator/transitions.js';
import {
  checkExperimentCompletion as checkExperimentCompletionForHost,
  handleSelectExperiment as handleSelectExperimentForHost,
  handleSpawnExperiments as handleSpawnExperimentsForHost,
  selectExperiment as selectExperimentForHost,
  selectExperiments as selectExperimentsForHost,
  type ExperimentDomainHost,
} from './orchestrator/experiments.js';
import {
  editTaskMergeMode as editTaskMergeModeForHost,
  type MergeDomainHost,
...
export class Orchestrator {
  ...
  private schedulingHost(): SchedulerDomainHost {
    return {
  ...
  private transitionHost(): TransitionDomainHost {
    return {
      ...this.schedulingHost(),
      deferredTaskIds: this.deferredTaskIds,
      invalidateLaunchArtifactsForTasks: (taskIds, reason, now) =>
        this.invalidateLaunchArtifactsForTasks(taskIds, reason, now),
    };
  }
  ...
  private experimentHost(): ExperimentDomainHost {
    return {
      stateMachine: this.stateMachine,
      persistence: this.persistence,
      logger: this.logger,
      ...
      autoStartReadyTasks: (taskIds, priority, opts) =>
        this.autoStartReadyTasks(taskIds, priority, opts),
  ...
  private mergeHost(): MergeDomainHost {
    return {
      persistence: this.persistence,
      refreshFromDb: () => this.refreshFromDb(),
      stateGetTask: (taskId) => this.stateGetTask(taskId),
      createTaskNotFoundError: (taskId) =>
        new OrchestratorError(OrchestratorErrorCode.TASK_NOT_FOUND, `Task ${taskId} not found`),
      cancelTask: (taskId) => this.cancelTask(taskId),
      dispatchPostMutation: (action, taskId) => this.dispatchPostMutation(action, taskId),
    };
  }
  ...
  private buildUpdateDelta(before: TaskState, after: TaskState, changes: TaskStateChanges): TaskDelta {
    return buildUpdateTaskDelta(before, after, changes);
  }

  private publishTaskDelta(delta: TaskDelta): void {
    publishTaskDeltaForHost(this.eventHost(), delta);
  }
  ...
  private publishTaskUpdate(
    ...
    return publishTaskUpdateForHost(this.eventHost(), before, after, changes, eventName);
  }

  private publishTaskRemoval(task: TaskState): TaskDelta {
    return publishTaskRemovalForHost(this.eventHost(), task);
  }
  ...
  private clearQueuedSchedulerEntries(taskId: string, attemptId?: string): void {
    clearQueuedSchedulerEntriesForHost(this.schedulingHost(), taskId, attemptId);
  }
  ...
  startExecution(): TaskState[] {
    return startExecutionForHost(this.schedulingHost());
  }
  ...
  selectExperiment(taskId: string, experimentId: string): TaskState[] {
    return selectExperimentForHost(this.experimentHost(), taskId, experimentId);
  }
  ...
  selectExperiments(
    ...
    return selectExperimentsForHost(
      this.experimentHost(),
      taskId,
      experimentIds,
      combinedBranch,
  ...
  editTaskMergeMode(
    ...
    return editTaskMergeModeForHost(this.mergeHost(), taskId, mergeMode);
  }
  ...
  deferTask(taskId: string): void {
    deferTaskForHost(this.transitionHost(), taskId);
  }
  ...
  getQueueStatus(): {
    ...
    return getQueueStatusForHost(this.schedulingHost());
  }
  ...
  private handleSpawnExperiments(
    ...
    return handleSpawnExperimentsForHost(this.experimentHost(), taskId, parsed);
  }
  ...
  private handleSelectExperiment(
    ...
    return handleSelectExperimentForHost(this.experimentHost(), taskId, parsed);
  }
  ...
  private checkExperimentCompletion(taskId: string): void {
    checkExperimentCompletionForHost(this.experimentHost(), taskId);
  }
  ...
  private autoStartReadyTasks(taskIds: string[], priority: number = 0, opts?: LaunchReadinessOptions): TaskState[] {
    return autoStartReadyTasksForHost(this.schedulingHost(), taskIds, priority, opts);
  }
  ...
  autoStartExternallyUnblockedReadyTasks(): TaskState[] {
    return autoStartExternallyUnblockedReadyTasksForHost(this.schedulingHost());
  }

  private autoStartUnblockedTasks(): TaskState[] {
    return autoStartUnblockedTasksForHost(this.schedulingHost());
  }

  getTaskLaunchReadiness(taskId: string, opts?: LaunchReadinessOptions): TaskLaunchReadiness {
    return getTaskLaunchReadinessForHost(this.schedulingHost(), taskId, opts);
  }
  ...
  private drainScheduler(): TaskState[] {
    return drainSchedulerForHost(this.schedulingHost());
  }
  ...
  markTaskRunningAfterLaunch(taskId: string, attemptId: string, launchedAt: Date = new Date()): boolean {
    return markTaskRunningAfterLaunchForHost(this.transitionHost(), taskId, attemptId, launchedAt);
  }
}
