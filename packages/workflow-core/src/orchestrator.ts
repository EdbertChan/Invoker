...
export class Orchestrator {
   ...
   * experiment selections are recreate-class invalidations
   * (`MUTATION_POLICIES.selectedExperiment.action === 'recreateTask'`).
   * Initial selection is not an invalidation because no downstream work
   * has consumed an earlier winner; same-winner re-selection is a no-op.
   *
   * On changed re-selection, active downstream work is canceled before
   * the reconciliation lineage is changed, then direct downstream
   * consumers are reset through `recreateTask`. That reset bumps
   * generation and clears stale branch/workspace/agent lineage so the
   * downstream subgraph reruns against the newly selected winner.
   */
  selectExperiment(taskId: string, experimentId: string): TaskState[] {
    ...
    const selectionInvalidationAction = MUTATION_POLICIES.selectedExperiment.action;
    if (isReSelection && selectionInvalidationAction !== 'recreateTask') {
      throw new Error(
        `selectExperiment expected recreateTask invalidation, got ${selectionInvalidationAction}`,
      );
    }
  ...
  selectExperiments(
    ...
    const sameAsPrev =
      ...
      prevCanon.every((id, i) => id === newCanon[i]);
    const isReSelection = previousSet !== undefined && !sameAsPrev;
    const selectionInvalidationAction = MUTATION_POLICIES.selectedExperimentSet.action;
    if (isReSelection && selectionInvalidationAction !== 'recreateTask') {
      throw new Error(
        `selectExperiments expected recreateTask invalidation, got ${selectionInvalidationAction}`,
      );
    }
