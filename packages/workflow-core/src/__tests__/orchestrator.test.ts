...
describe('Orchestrator', () => {
  ...
  describe('editTaskCommand', () => {
    ...
    // ── Step 2 (task-invalidation roadmap): recreate-class semantics ──
    //
    // The chart's Decision Table row "Edit `command`" maps the command
    // mutation to InvalidationAction = 'recreateTask' with
    // InvalidationScope = 'task'. The orchestrator method enforces
    // cancel-first via cancelTask BEFORE the lineage-discarding
    // recreateTask reset (the synchronous orchestrator-internal
    // equivalent of applyInvalidation's cancelInFlight dep). The
    // following tests pin those invariants.
  ...
  describe('Step 12: workflow-scope paths', () => {
    ...
    describe('applyInvalidation routing (Step 11 "not yet wired" path is closed)', () => {
      it("applyInvalidation('workflow', 'recreateWorkflowFromFreshBase', ...) succeeds when the dep is wired (no longer throws \"not yet wired (Step 12)\")", async () => {
  ...
  describe('selectExperiments (Step 8 invalidation)', () => {
    ...
    it('Step 8: re-selecting CHANGED merged set with an ACTIVE downstream cancels first, then retry-class resets via retryTask', () => {
      ...
      // cancel-first MUST precede the retry-class downstream reset,
      // and recreateTask MUST NOT be on the route (merged-set
      // selection is retry-class per the Decision Table row "Edit
      ...
      const restartOrder = restartSpy.mock.invocationCallOrder[0];
    ...
    it('Step 8: INITIAL multi-select (recon needs_input → completed) does NOT cancel and does NOT restart downstream', () => {
    ...
    it('Step 8: re-selection bumps each affected downstream execution generation by exactly one', () => {
      const { reconId, exp1Id, exp2Id, exp3Id, directDownstream } =
        setupMergedReconciliationWithDownstream();
    ...
    it('Step 8: re-selecting the SAME merged set is a no-op (no cancel, no restart)', () => {
      const { reconId, exp1Id, exp2Id, directDownstream } =
        setupMergedReconciliationWithDownstream();
    ...
    it('Step 8: same merged set in different ORDER is still treated as the same set (no cancel, no restart)', () => {
  ...
  describe('editTaskFixContext (Step 10 invalidation)', () => {
    ...
    it('Step 10: edit on ACTIVE fix session reverts to failed-baseline — agentSessionId cleared, new fix prompt persisted', () => {
    ...
    it('Step 10: same-content edit is a NO-OP (no cancel, no restart, no generation bump, no delta)', () => {
      const { taskId, workflowId } = setupFailedTask();
    ...
    it('Step 10: omitted patch key leaves the existing config field untouched (prompt-only edit preserves context)', () => {
      const { taskId, workflowId } = setupFailedTask();
      persistence.updateTask(taskId, {
    ...
    it('Step 10: content change bumps execution generation by exactly one', () => {
      const { taskId } = setupFailedTask();
      const before = orchestrator.getTask(taskId)!.execution.generation ?? 0;
    ...
    it('Step 10: emits a `task.updated` delta carrying the new fix prompt/context', () => {
      const { taskId } = setupFailedTask();
      publishedDeltas = [];
    ...
    it('Step 10: throws when called on a merge node', () => {
      orchestrator.loadPlan({
    ...
    it('Step 10: throws when called on an unknown task id', () => {
      setupFailedTask();
      expect(() =>
    ...
    it('Step 10: throws when called on a task whose status is neither failed nor fixing_with_ai', () => {
      orchestrator.loadPlan({
