...
import type { GraphMutation } from './orchestrator.js';
...
export function reconcileMergeLeavesImpl(host: GraphMutationHost, workflowId: string): void {
  ...
  host.publishDelta(host.buildUpdateDelta(mergeNode, updated, changes));
}
...
export function applyGraphMutationImpl(host: GraphMutationHost, mutation: GraphMutation): TaskDelta[] {
  ...
  for (const task of allTasks) {
    ...
    const delta = host.buildUpdateDelta(task, remapped, remapChanges);
  ...
  const sourceDelta: TaskDelta = sourceTaskBefore
    ? host.buildUpdateDelta(sourceTaskBefore, updatedSource, sourceChanges)
    : {
        type: 'updated',
  ...
  for (const nodeDef of mutation.newNodes) {
    ...
    host.logTaskEvent(task.id, 'task.created');
    host.publishDelta(delta);
    allDeltas.push(delta);
  }
