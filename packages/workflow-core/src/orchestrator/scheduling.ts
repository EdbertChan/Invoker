...
export function autoStartUnblockedTasks(host: SchedulerDomainHost): TaskState[] {
  for (const task of host.stateMachine.getAllTasks()) {
    ...
    if (getLocalDependencyBlocker(host, task)) continue;
    if (host.getExternalDependencyBlocker(task) !== undefined) continue;
...
export function getLocalDependencyBlocker(
  host: Pick<SchedulerDomainHost, 'stateGetTask'>,
  task: TaskState,
): string | undefined {
...
export function getQueueStatus(host: SchedulerDomainHost): {
  maxConcurrency: number;
  runningCount: number;
  ...
  host.refreshFromDb();
  const tasks = host.stateMachine.getAllTasks();
  const now = Date.now();
  const activeAttempts = tasks
    ...
    .filter(({ task, attempt }) => host.isTaskExecutionActive(task, attempt, now));
  const activeTaskIds = new Set(activeAttempts.map(({ task }) => task.id));
  const queuedTasks = host.stateMachine
    .getReadyTasks()
