...
export interface SchedulerDomain {
  enqueueIfNotScheduled(taskId: string, priority?: number): void;
...
export function createSchedulerDomain(host: SchedulerDomainHost): SchedulerDomain {
  function drainScheduler(): TaskState[] {
    ...
    while (job && availableSlots > 0) {
      const task = host.getTask(job.taskId);
      ...
      const selectedTask = host.getTask(job.taskId) ?? task;
      ...
      host.publishTaskDelta(host.buildUpdateDelta(task, updated, changes));
  ...
  function enqueueIfNotScheduled(taskId: string, priority: number = 0): void {
  ...
  function autoStartReadyTasks(taskIds: string[], priority: number = 0): TaskState[] {
    for (const taskId of taskIds) {
      let task = host.getTask(taskId);
      if (!task) continue;
      ...
      if (task.status === 'blocked') {
        ...
        task = host.getTask(taskId);
        if (!task) continue;
      }

      enqueueIfNotScheduled(taskId, priority);
    }

    return drainScheduler();
  }

  function autoStartExternallyUnblockedReadyTasks(): TaskState[] {
    ...
    for (const task of readyTasks) {
      enqueueIfNotScheduled(task.id);
    }
    return drainScheduler();
  }

  function autoStartUnblockedTasks(): TaskState[] {
  ...
  return {
    enqueueIfNotScheduled,
    autoStartReadyTasks,
    autoStartUnblockedTasks,
