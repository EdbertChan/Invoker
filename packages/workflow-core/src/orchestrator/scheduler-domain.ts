...
export function autoStartUnblockedTasks(host: SchedulerDomainHost): TaskState[] {
  for (const task of host.getAllTasks()) {
    ...
    if (!areLocalDependenciesSatisfied(host, task)) continue;
    if (host.getExternalDependencyBlocker(task) !== undefined) continue;
...
export function getTaskLaunchReadiness(
  ...
  if (!opts?.bypassLocalDependencyReadiness) {
    const localBlocker = getLocalDependencyBlocker(host, task);
    if (localBlocker) {
...
export function areLocalDependenciesSatisfied(host: SchedulerDomainHost, task: TaskState): boolean {
  return getLocalDependencyBlocker(host, task) === undefined;
}

export function getLocalDependencyBlocker(host: SchedulerDomainHost, task: TaskState): string | undefined {
...
export function drainDeferredTasks(host: SchedulerDomainHost): TaskState[] {
