...
export class CommandService {
   ...
   * lineage or `recreateTask` to discard it. This shim follows the
   * INV-91 experiment decision and delegates to `retryTask`, matching
   * the legacy task restart behavior exposed by IPC and HTTP.
   */
  async restartTask(
    envelope: CommandEnvelope<{ taskId: string }>,
  ): Promise<CommandResult<TaskState[]>> {
    console.warn(
      `[command-service] restartTask("${envelope.payload.taskId}") is deprecated (Step 13). ` +
        'Routing to retryTask. Use retryTask/recreateTask explicitly.',
    );
    return this.retryTask(envelope);
  }
