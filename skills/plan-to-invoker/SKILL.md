...
**Review strategy rule:** The execution engine routes review publication through a **strategy router** (`packages/execution-engine/src/publication-strategy-router.ts`). Two strategies are supported:
...
10. `step-submit` (no stacking)
    ...
    If the workflow's `reviewStrategy` is `mergify_stack` (e.g. Invoker-on-Invoker), publish the PR stack with `mergify stack push` after commits are ready. Otherwise the engine uses `github_pr` (default) automatically.
10a. `step-submit-stacked` (single plan with upstream dependency)
     ...
     5. If `reviewStrategy: mergify_stack`, publish/update the resulting PR stack with `mergify stack push` after submission-side commits are ready.
10b. `step-submit-chain` (batch stacking, multiple template plans)
     ...
     The chain script handles: template rendering, baseBranch rewrite, merge-gate injection, sequential submission. When `reviewStrategy: mergify_stack`, publish the resulting PR stack with `mergify stack push` once the chain's commits are prepared. For `github_pr` (default), the engine handles PR creation automatically.
...
When those hardening workflows target Invoker itself (`reviewStrategy: mergify_stack`), publish the resulting PR stack with `mergify stack push` after the commits are ready. Workflows targeting external repos use `github_pr` (default) unless the repo independently opts into `mergify_stack`.
