...
function validatePlan(yamlContent: string): ValidationError[] {
  ...
  // Validate visual proof requirements
  const uiPattern = /packages\/ui\/|@invoker\/ui/;
  const referencesUi = raw.tasks.some((task: RawPlanTask) =>
    uiPattern.test(task.prompt || '') ||
    uiPattern.test(task.description || '') ||
    uiPattern.test(task.command || ''),
  );
  ...
  if (raw.visualProof === true) {
    const hasVisualProofTestTask = raw.tasks.some((task: RawPlanTask) =>
    ...
    if (!hasVisualProofTestTask) {
      errors.push({
        ...
        field: 'tasks',
        message: 'Plan sets visualProof: true but no task has a prompt referencing "visual-proof.spec.ts" or "captureScreenshot". Add a prompt task that generates or runs visual proof tests.',
      });
    }

    const hasVisualProofCaptureTask = raw.tasks.some((task: RawPlanTask) =>
    ...
    if (!hasVisualProofCaptureTask) {
      errors.push({
        ...
        field: 'tasks',
        message: 'Plan sets visualProof: true but no task has a command referencing "ui-visual-proof.sh". Add a command task that captures visual proof.',
      });
    }
  }
