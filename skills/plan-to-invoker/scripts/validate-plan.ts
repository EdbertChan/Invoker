...
function validatePlan(yamlContent: string): ValidationError[] {
  ...
  const hasUiReferences = raw.tasks.some((task: RawPlanTask) => (
    uiReferencePattern.test(task.prompt || '') ||
  ...
  if (raw.visualProof === true) {
    ...
    if (!hasVisualProofTestTask) {
      errors.push({
        ...
        field: 'tasks',
        message: 'Plan sets visualProof: true but does not include a prompt task referencing visual-proof.spec.ts or captureScreenshot. Add a prompt task for visual proof testing.',
      });
    }
    ...
    if (!hasVisualProofCaptureTask) {
      errors.push({
        ...
        field: 'tasks',
        message: 'Plan sets visualProof: true but does not include a command task invoking ui-visual-proof.sh. Add a command task to capture visual proof.',
      });
    }
  }
