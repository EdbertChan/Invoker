# Schema Reference — Field Decisions

Source of truth: `packages/app/src/plan-parser.ts` (interfaces + validation, lines 21-175)

## Plan-Level Fields

| Field | Type | Default | When to set |
|-------|------|---------|-------------|
| `name` | string | *required* | Always. Short, descriptive. |
| `description` | string | — | **Required** when onFinish is pull_request or merge. Validator rejects plans without it. 1-3 paragraphs: architecture, motivations, tradeoffs. Include mermaid before/after architecture diagrams when the change modifies component interactions or data flow (GitHub renders them natively in the PR body). See `examples.md` Example 4. |
| `visualProof` | boolean | false | Set true when plan modifies UI packages (packages/ui/). Triggers automatic before/after screenshot capture in the merge gate. |
| `onFinish` | `pull_request` \| `merge` \| `none` | `pull_request` | `pull_request` for code changes needing review. `none` for tests/diagnostics/verification. `merge` rarely — auto-merge after completion. |
| `baseBranch` | string | auto-detected | Omit unless targeting a non-default branch. |
| `featureBranch` | string | auto-generated from name | Omit — auto-generates as `plan/<slug>`. Set only to target a specific branch. |
| `approvalMode` | `manual` \| `automatic` \| `external_review` | `manual` (parser) | Parser default is `manual`. **Plans generated by this skill:** implementation YAML should set `approvalMode: external_review` (external review gate in UI) unless the user requests `manual` or `automatic`. Verify-only (`onFinish: none`) plans usually stay `manual`. |
| `reviewStrategy` | `github_pr` \| `mergify_stack` | `github_pr` (router) | Controls which `MergeGateProvider` the execution engine uses for review creation and approval polling. `github_pr` (default): creates a standard GitHub PR via the GitHub provider. `mergify_stack` (explicit opt-in): runs `mergify stack push` and resolves the resulting stacked PR. Set `mergify_stack` only when the target repo uses Mergify Stacks (e.g. Invoker-on-Invoker dogfooding). Set at workflow persistence level (see `packages/workflow-core/src/orchestrator.ts`). The execution engine resolves strategy via `publication-strategy-router.ts`. |
| `repoUrl` | string | *required* (parser) | **Always** set the canonical git remote for the repo where tasks run (SSH or HTTPS), e.g. output of `git remote get-url origin` in that checkout. `plan-parser` rejects plans without `repoUrl`. |
| `executorType` | string | — | Top-level default inherited by all tasks. Set per-task instead when mixed. |

## Task-Level Fields

| Field | Type | Default | When to set |
|-------|------|---------|-------------|
| `id` | string | *required* | Always. Kebab-case, descriptive: `implement-auth-middleware`, `test-auth-middleware`. |
| `description` | string | *required* | Always. What the task does and why. May include **optional** multi-line sections for handoff: `Files:`, `Change types:`, `Acceptance criteria:` (best-effort hints; see `references/task-patterns.md`). Not validated by `plan-parser.ts`. |
| `command` | string | — | Task runs a shell command. Mutually exclusive with `prompt`. |
| `prompt` | string | — | Task needs LLM reasoning. Include file paths, line numbers, specific instructions. Mutually exclusive with `command`. |
| `dependencies` | string[] | `[]` | Always include, even if empty. List task IDs that must complete first. |
| `executorType` | `worktree` \| `docker` \| `ssh` | `worktree` | `worktree` for tasks that run in a git worktree (default). `docker` for isolated environments. `ssh` for remote execution via SSH targets. |
| `remoteTargetId` | string | — | Required when `executorType: ssh`. Must match a key in `~/.invoker/config.json` `remoteTargets`. |
| `dockerImage` | string | — | Required when `executorType: docker`. |
| `autoFix` | boolean | `false` | `true` for tasks that should auto-retry with experiments on failure. |
| `pivot` | boolean | `false` | `true` to spawn experiment variants. Requires `experimentVariants`. |
| `requiresManualApproval` | boolean | `false` | `true` for destructive or irreversible tasks. |

## Decision Table

| Situation | Field | Value |
|-----------|-------|-------|
| Code changes that need review | `onFinish` | `pull_request` |
| Same, via this skill (implementation YAML) | `approvalMode` | `external_review` unless user overrides |
| Same, and the target repo is Invoker itself | `reviewStrategy` | `mergify_stack` (workflow-level); publish with `mergify stack push` after the commit stack is ready |
| External repo without Mergify Stacks | `reviewStrategy` | omit (defaults to `github_pr`) |
| Tests, diagnostics, verification | `onFinish` | `none` |
| Auto-merge after completion | `onFinish` | `merge` |
| Task runs a shell command | `command` | the command |
| Task needs LLM reasoning | `prompt` | instructions with file paths |
| Task modifies code via LLM | `executorType` | `worktree` |
| Task is read-only / test | `executorType` | omit (defaults to `worktree`) |
| Task should auto-retry on failure | `autoFix` | `true` |
| Every generated plan | `repoUrl` | real remote URL from `git remote get-url origin` (or equivalent) |

## Implementation plans (convention)

`plan-parser.ts` does not require a special field for this, but **implementation** YAML (not one-off verify-only plans) must reserve the **last** task for the terminal regression gate: a `command` task that runs `pnpm run test:all`. Keep focused package/headless repro commands earlier in the plan. The final `pnpm run test:all` task must depend on every earlier task so it runs last. See `references/task-patterns.md` and `playbooks/verify-then-build.md` Phase 2.

## Banned Patterns

These will cause validation failures:

- **`npx vitest run`** — parser rejects it (ABI mismatch). Use `cd packages/<pkg> && pnpm test`.
- **`pnpm test <path>` from repo root** — runs `pnpm -r test` across all packages. Always `cd` into the package first.
- **Invented test filenames** — verify they exist before referencing them in commands.
- **`command` + `prompt` on same task** — pick one. Shell logic → `command`. Reasoning → `prompt`.

## Valid Enum Values

- `onFinish`: `none`, `merge`, `pull_request`
- `approvalMode`: `manual`, `automatic`, `external_review`

## Review Strategy Architecture

The execution engine uses a **publication strategy router** (`packages/execution-engine/src/publication-strategy-router.ts`) to dispatch review creation and approval polling to the correct provider.

| Strategy key | Provider | Behavior | When to use |
|-------------|----------|----------|-------------|
| `github_pr` (default) | `GitHubMergeGateProvider` | Creates a standard GitHub PR, polls for review approval | All repos unless they opt into Mergify Stacks |
| `mergify_stack` | `MergifyStackProvider` | Runs `mergify stack push`, resolves stacked PR, polls approval | Repos that use Mergify Stacks (e.g. Invoker-on-Invoker) |

**Resolution order:** strategy key → built-in mapping → `ReviewProviderRegistry`.

`approvalMode: external_review` controls Invoker's review gate behavior (external review gate in UI). `reviewStrategy` controls which provider handles the review lifecycle. These are orthogonal: `approvalMode: external_review` + `reviewStrategy: github_pr` is the standard path; `approvalMode: external_review` + `reviewStrategy: mergify_stack` is the stacked publication path.

**Known `mergify_stack` limitations** (from lifecycle PoC, `docs/mergify-stack-lifecycle-poc.md`):
- Mid-stack rewrites recreate affected PRs with new numbers; review comments are lost.
- Re-push after a mid-stack cancel fails with HTTP 422; adapter must close downstream PRs or retarget bases before re-pushing.
- PR number instability requires refreshing PR mappings after every force-push.

- `executorType`: `worktree`, `docker`, `ssh`
