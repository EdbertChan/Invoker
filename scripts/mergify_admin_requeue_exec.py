...
def load_candidate_stacks(
    ...
    candidate_numbers = {int(pr.get("number") or 0) for pr in candidates if int(pr.get("number") or 0)}
    if not candidate_numbers:
        return ()

    # Prefer a full open-PR listing for base/head stack discovery. If the listing
    # is unavailable (older fake-gh stubs, transient gh failures), fall back to
    # the candidate set which already expands known stack metadata members.
    try:
        open_prs = gh.list_open_prs(repo)
    except Exception:
        open_prs = []
    if not open_prs:
        open_prs = list(candidates)
    ...
    raw_by_number.update({int(pr.get("number") or 0): pr for pr in candidates if int(pr.get("number") or 0)})
