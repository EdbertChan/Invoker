#!/usr/bin/env bash
# Repro script: SSH managed fan-in must not merge the synthetic workflow base as a dependency.
#
...
#
# Correct behavior:
#   setupTaskBranch skips the synthetic base and merges only real dependency
#   branches.
#
# Usage:
...
TEST_NAME="skips synthetic fan-in base when an explicit resolved base is supplied for worktree setup"
