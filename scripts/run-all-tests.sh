...
if [ "$PROOF" = "1" ]; then
  # INV-117 proof mode implements docs/context/inv-117/experiment-brief.md:
  # rerun every discovered suite, disable checkpoint reuse, and validate the
  # deterministic summary thresholds before reporting success.
  FORCE_RERUN=1
  RESUME=0
  JOBS="${INVOKER_TEST_ALL_JOBS:-1}"
fi
