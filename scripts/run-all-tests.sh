...
INV_117_REQUIRED_EXPECTED=16
INV_117_EXTENDED_EXPECTED=23
INV_117_DANGEROUS_EXPECTED=24
INV_117_DANGEROUS_DOCKER_UNAVAILABLE_EXPECTED=23
INV_117_DANGEROUS_DOCKER_SUITE="dangerous/10-docker-comprehensive.sh"
...
expected_executed_for_mode() {
  case "$MODE_KEY" in
    required)
      printf '%s' "$INV_117_REQUIRED_EXPECTED"
      ;;
    extended)
      printf '%s' "$INV_117_EXTENDED_EXPECTED"
      ;;
    dangerous)
      if [ "${#SKIPPED_UNAVAILABLE[@]}" -eq 1 ] && [ "${SKIPPED_UNAVAILABLE[0]}" = "$INV_117_DANGEROUS_DOCKER_SUITE" ]; then
        printf '%s' "$INV_117_DANGEROUS_DOCKER_UNAVAILABLE_EXPECTED"
      else
        printf '%s' "$INV_117_DANGEROUS_EXPECTED"
      fi
      ;;
  esac
}
...
validate_proof_thresholds() {
  ...
  case "$MODE_KEY" in
    ...
    dangerous)
      ...
      if [ "${#SKIPPED_UNAVAILABLE[@]}" -eq 1 ] && [ "${SKIPPED_UNAVAILABLE[0]}" != "$INV_117_DANGEROUS_DOCKER_SUITE" ]; then
        echo "ERROR: INV-117 proof only allows unavailable skip for $INV_117_DANGEROUS_DOCKER_SUITE" >&2
        return 1
      fi
      ;;
  esac
