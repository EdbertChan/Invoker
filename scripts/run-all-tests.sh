...
PROOF_CONTRACT_ID="INV-117"
PROOF_CONTRACT_DOC="docs/context/inv-117/experiment-brief.md"
PROOF_REQUIRED_EXECUTED=16
PROOF_EXTENDED_EXECUTED=23
PROOF_DANGEROUS_EXECUTED=24
PROOF_DANGEROUS_EXECUTED_WITH_DOCKER_UNAVAILABLE=23
...
expected_executed_for_mode() {
  case "$MODE_KEY" in
    required)
      printf '%s' "$PROOF_REQUIRED_EXECUTED"
      ;;
    extended)
      printf '%s' "$PROOF_EXTENDED_EXECUTED"
      ;;
    dangerous)
      if [ "${#SKIPPED_UNAVAILABLE[@]}" -eq 1 ] && [ "${SKIPPED_UNAVAILABLE[0]}" = "dangerous/10-docker-comprehensive.sh" ]; then
        printf '%s' "$PROOF_DANGEROUS_EXECUTED_WITH_DOCKER_UNAVAILABLE"
      else
        printf '%s' "$PROOF_DANGEROUS_EXECUTED"
      fi
      ;;
  esac
}
...
validate_proof_thresholds() {
  ...
  if [ "${#EXECUTED[@]}" -ne "$expected_executed" ]; then
    echo "ERROR: $PROOF_CONTRACT_ID proof expected Executed=$expected_executed from $PROOF_CONTRACT_DOC, got ${#EXECUTED[@]}" >&2
    return 1
  fi

  if [ "${#FAILED[@]}" -ne 0 ]; then
    echo "ERROR: $PROOF_CONTRACT_ID proof expected Failed=0 from $PROOF_CONTRACT_DOC, got ${#FAILED[@]}" >&2
    return 1
  fi

  if [ "${#SKIPPED_CHECKPOINT[@]}" -ne 0 ]; then
    echo "ERROR: $PROOF_CONTRACT_ID proof expected Skipped by checkpoint=0 from $PROOF_CONTRACT_DOC, got ${#SKIPPED_CHECKPOINT[@]}" >&2
    return 1
  fi
  ...
  case "$MODE_KEY" in
    required|extended)
      if [ "${#SKIPPED_UNAVAILABLE[@]}" -ne 0 ]; then
        echo "ERROR: $PROOF_CONTRACT_ID proof expected Skipped unavailable=0 from $PROOF_CONTRACT_DOC, got ${#SKIPPED_UNAVAILABLE[@]}" >&2
        return 1
      fi
      ;;
    dangerous)
      if [ "${#SKIPPED_UNAVAILABLE[@]}" -gt 1 ]; then
        echo "ERROR: $PROOF_CONTRACT_ID proof expected at most one unavailable skip from $PROOF_CONTRACT_DOC, got ${#SKIPPED_UNAVAILABLE[@]}" >&2
        return 1
      fi
      if [ "${#SKIPPED_UNAVAILABLE[@]}" -eq 1 ] && [ "${SKIPPED_UNAVAILABLE[0]}" != "dangerous/10-docker-comprehensive.sh" ]; then
        echo "ERROR: $PROOF_CONTRACT_ID proof only allows unavailable skip for dangerous/10-docker-comprehensive.sh from $PROOF_CONTRACT_DOC" >&2
        return 1
      fi
      ;;
  esac
