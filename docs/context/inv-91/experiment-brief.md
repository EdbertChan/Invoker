...
**Alternative A selected.** Per-step blast radius of 1-2 files vs 12-13 files. Each step independently revertable. Zero runtime crash risk during migration. The incremental approach is strictly safer for a deprecation spanning 9 layers.
...
| `packages/app/src/headless.ts` | app | Headless CLI; `rebase-and-retry` command |
| `packages/app/src/headless-command-classification.ts` | app | Command classifier; `rebase-and-retry` entry |
| `packages/app/src/headless-delegation.ts` | app | Headless delegation; `restart-task` envelope |
...
- **Baseline (pre-migration):** ~38 occurrences across 10 prod files.
- **Threshold:** 0 occurrences in production `src/` files.
...
```bash
rg -c 'restart.*404\|rebase-and-retry.*404' \
...
- **Pass condition:** All restart/rebase-and-retry test lines show pass, not fail.
...
Captured 2026-05-09. These values represent the pre-migration state on the current branch (based on master).
...
| M2 (canonical symbols) | Present in orchestrator.ts (retryTask line 2028, recreateTask referenced), ipc-channels.ts (invoker:recreate-task line 327, invoker:retry-workflow line 331), api-server.ts (retry/recreate routes) | Direct file inspection |
| M3 (workflow-core tests) | Pass | Prior workflow run |
| M3 (contracts tests) | Pass | Prior workflow run |
| M3 (app tests) | Pass | Prior workflow run |
| M5 (full regression) | Pass | Prior workflow run |
...
The implementation at commit `9f6d1e32` on branch `intermediate/experiment/wf-1778141629042-19/implement-inv-91/` is the reference artifact for the selected approach.
