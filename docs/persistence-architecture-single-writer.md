...
| `invoker:rebase-and-retry` | main.ts:1094 | N/A (owner) | `rebaseAndRetry()` → persistence + orchestrator | Canonical name: `recreate-with-rebase` (workflow-scoped) |
...
| `rebase` | headless.ts:754 | **Yes** (line 365) | `tryDelegateExec()` OR standalone writable | Workflow-scoped `recreate-with-rebase wf-…` and deprecated `rebase-and-retry wf-…` delegates use a 60s timeout; task-scoped rebase stays at 5s |
...
2. **Headless delegates by default**: When GUI is present, headless commands try delegation first. `run`, `resume`, and most `headless.exec` commands use a 5s timeout; workflow-scoped `recreate-with-rebase`, `rebase`, `rebase-and-retry`, and `restart` use 60s. Only if delegation fails (no GUI or timeout) does headless open its own writable DB.
...
5. **Delegation timeout prevents deadlock**: IPC delegation is bounded so headless does not hang if GUI is unresponsive. The default is 5s, with a 60s allowance for workflow-scoped `recreate-with-rebase`, `rebase`, `rebase-and-retry`, and `restart` command shapes in `headless.exec`.
